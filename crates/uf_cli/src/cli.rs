...
pub(crate) enum Commands {
    ...
    /// JSDoc blocks attached to exported declarations, and writes an offline
    /// documentation site with uf's design and ox-content's Markdown renderer.
    Doc {
        /// Directory to receive the site and `api.md`.
        #[arg(
            long = "out",
            alias = "out-dir",
            value_name = "DIR",
        ...
        out_dir: Utf8PathBuf,
        /// Output an HTML site or only the Markdown source.
        #[arg(long, value_enum, default_value = "html")]
        format: DocFormat,
        /// Emit the report as JSON without writing files.
        #[arg(long)]
        json: bool,
    },
    ...
    Test {
        ...
        ///
        /// Left out, the session listens for the operating system's file
        /// events instead (FSEvents, inotify), and polls only where those do
        /// not arrive — as often as costs about two per cent of a core, every
        /// 50 ms on a few hundred files, backing off to 250 ms on a project
        /// large enough that a look takes longer.
...
/// The output generated by `uf doc`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub(crate) enum DocFormat {
    /// An offline site with uf styling and its Markdown source.
    Html,
    /// Only the Markdown API reference.
    Markdown,
}
