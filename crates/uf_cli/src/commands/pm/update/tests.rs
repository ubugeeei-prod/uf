...
fn one_dependency_reads_as_one() {
    ...
    assert!(out.contains("1 dependency is newer than its range allows"), "{out}");
}
...
fn a_level_report_shows_the_range_it_writes() {
    ...
    assert!(out.contains("^19.2.0"), "the rewritten range is missing: {out}");
...
fn a_project_uf_could_not_ask_about_is_not_told_it_is_current() {
    ...
    assert!(out.contains("no dependency declares a range uf can compare"), "{out}");
...
fn an_unreachable_package_is_named_rather_than_counted() {
    let mut report = report(None, Vec::new());
    report.unreachable = vec!["left-pad: could not read left-pad from the registry: 404".to_owned()];
...
fn the_rows_that_need_a_decision_are_first() {
    ...
    rows.sort_by(|left, right| right.step.cmp(&left.step).then_with(|| left.name.cmp(&right.name)));
