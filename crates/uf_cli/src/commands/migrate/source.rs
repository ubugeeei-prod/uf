...
pub(super) fn object(source: &str) -> Result<Object> {
    let parsed = uf_flow::validate_source(source)?;
        ...
        parsed.is_ok(),
        uf_infra::cstr!("configuration does not parse: {:?}", parsed.diagnostics)
    );
    outline::read(source).ok_or_else(|| anyhow::anyhow!(uf_infra::cstr!("expected an exported object or defineConfig(object); dynamic configuration needs manual migration")))
}

fn entry<'a>(source: &str, object: &'a Object, key: &str) -> Result<Option<&'a Entry>> {
    ensure!(
        object.entries.iter().all(|e| e.key.is_some()),
        uf_infra::cstr!("computed keys, methods and spreads need manual migration")
    );
    ...
    let first = entries.next();
        ...
        uf_infra::cstr!("duplicate config key {key}")
    );
    Ok(first)
}
...
pub(super) fn at<'a>(source: &str, root: &'a Object, path: &[&str]) -> Result<Option<&'a Value>> {
    let Some((key, rest)) = path.split_first() else {
        bail!(uf_infra::cstr!("empty config path"))
    };
    ...
    let Value::Object(object) = value else {
        bail!(uf_infra::cstr!("{} is not a static object", key))
    };
...
pub(super) fn put_raw(source: &mut String, path: &[&str], value: &str) -> Result<()> {
    let (key, parents) = path
        .split_last()
        .ok_or_else(|| anyhow::anyhow!(uf_infra::cstr!("empty config path")))?;
    ...
    let parent = if parents.is_empty() {
        ...
        match at(source, &root, parents)? {
            ...
            _ => bail!(uf_infra::cstr!("{} is not an object", parents.join("."))),
        }
    };
    if let Some(existing) = entry(source, parent, key)? {
        ...
        source.insert_str(
            parent.open + 1,
            uf_infra::cstr!("\n{indent}  {}: {value},", serde_json::to_string(key)?).as_str(),
        );
    }
    object(source)?;
    Ok(())
