...
mod site;
pub use site::{render_html, write_site};
...
#[derive(Debug, Error)]
pub enum DocError {
    /// The generated Markdown could not be parsed by ox-content.
    #[error("failed to render documentation: {0}")]
    Markdown(#[from] ox_content_parser::ParseError),
...
pub fn render_markdown(report: &DocReport) -> String {
    render_markdown_with(report, |_| {})
}

pub(crate) fn render_markdown_with(report: &DocReport, mut on_module: impl FnMut(usize)) -> String {
