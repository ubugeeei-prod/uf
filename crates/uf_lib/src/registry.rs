...
///
/// `ContextMenu`, `Menubar` and `RangeCalendar` are namespaces whose modules
/// re-export another module's parts — `Menu`'s, `Calendar`'s — so each reaches
/// that module as well as its own. `toast` and its
...
const BARREL_EXPORT_MODULES: &[(&str, &[&str])] = &[
    ...
    ("RangeCalendar", &["calendar", "range-calendar"]),
    ("parseColor", &["color-picker"]),
    ("parseNumber", &["number-field"]),
    ("startsWithLocale", &["i18n-provider"]),
    ("useCollator", &["i18n-provider"]),
    ("useDragAndDrop", &["drag-drop"]),
    ("useFilter", &["i18n-provider"]),
    ("useLocale", &["i18n-provider"]),
    ...
    ("useInteractOutside", &["interactions"]),
...
/// its modules': a namespace is its module in PascalCase, re-exported whole
/// (`export * as AlertDialog from "./alert-dialog.js"`, ubugeeei-prod/uf#1453),
/// a single component is named the same way (`Switch` is `switch`), and where
/// two modules fit, the longer one is where the name comes from (`ToggleGroup` is
/// `toggle-group`, not `toggle`). A name from a Server Component module, such as
