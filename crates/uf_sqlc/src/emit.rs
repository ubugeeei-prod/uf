...
pub fn generate(request: &GenerateRequest) -> Result<Vec<File>, String> {
    ...
    let qualify = |schema: &str, name: &str| {
        if schema == default_schema || schema.is_empty() {
            ...
            compact_str::format_compact!("{schema}_{name}").into_string()
        }
    };
    ...
    for schema in &catalog.schemas {
        ...
        for entry in &schema.enums {
            ...
            enums.push(EnumInfo {
                ...
                values_name: compact_str::format_compact!("{base}Values").into_string(),
                codec_name: compact_str::format_compact!("{base}Codec").into_string(),
                type_name,
    ...
    for (filename, queries) in by_file {
        ...
        files.push(File {
            name: compact_str::format_compact!("{stem}.js").into_string(),
            contents: module.finish(compact_str::format_compact!("from `{stem}`").as_str()),
        });
    }
    Ok(files)
}
...
fn doc(lines: &[String], indent: &str) -> String {
    ...
    let mut out = compact_str::format_compact!("{indent}/**\n").into_string();
    for line in lines {
...
fn header(shared: &Shared<'_>, what: &str) -> String {
    ...
    let with = if version.is_empty() {
        ...
        compact_str::format_compact!(" with sqlc {version}").into_string()
    };
...
fn models_file(shared: &Shared<'_>) -> Result<String, String> {
    ...
    for model in &shared.models {
        ...
        let table_label = if model.schema == shared.default_schema {
            ...
            compact_str::format_compact!("{}.{}", model.schema, model.sql_name).into_string()
        };
        ...
        module
            .body
            .push_str(compact_str::format_compact!("/** A row of `{table_label}`. */\n").as_str());
        uf_infra::append!(
            module.body,
            "export type {} = {{|\n{}|}};\n\n",
            model.type_name,
...
fn unique(names: &mut BTreeSet<String>, name: String) -> String {
    ...
    for n in 2.. {
        let candidate = compact_str::format_compact!("{name}{n}").into_string();
...
impl<'a> Module<'a> {
    ...
    fn slug(codec: &Codec) -> String {
        match codec {
            ...
            Codec::Array(inner) => {
                compact_str::format_compact!("{}_array", Self::slug(inner)).into_string()
            }
    ...
    fn hoist(&mut self, slug: &str, expression: String) -> String {
        ...
        let base = compact_str::format_compact!("{}Codec", camel(slug)).into_string();
        let mut name = base.clone();
        let mut n = 2;
        while self.taken.contains(&name) {
            name = compact_str::format_compact!("{base}{n}").into_string();
            n += 1;
        }
    ...
    fn codec(&mut self, codec: &Codec) -> String {
        ...
        match codec {
            Codec::Export(name) => compact_str::format_compact!("{alias}.{name}").into_string(),
            ...
            Codec::Array(inner) => {
                ...
                self.hoist(
                    &Self::slug(codec),
                    compact_str::format_compact!("{alias}.array({inner_ref})").into_string(),
                )
            }
    ...
    fn decode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return compact_str::format_compact!("{alias}.unknown.decode({value})").into_string();
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            compact_str::format_compact!("{alias}.nullable({codec}, {value})").into_string()
        } else {
            compact_str::format_compact!("{codec}.decode({value})").into_string()
        }
    }
    ...
    fn encode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return compact_str::format_compact!("{alias}.unknown.encode({value})").into_string();
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            compact_str::format_compact!("{alias}.param({codec}, {value})").into_string()
        } else {
            compact_str::format_compact!("{codec}.encode({value})").into_string()
        }
    }
        ...
        for column in &query.columns {
            if let Some(embed) = &column.embed_table {
                ...
                for table_column in &model.columns {
                    ...
                    let decode =
                        self.decode(&mapped, compact_str::format_compact!("row[{at}]").as_str());
                    inner.push(
                        compact_str::format_compact!("{}: {decode}", key(&name)).into_string(),
                    );
                    at += 1;
                }
            ...
            let decode = self.decode(&mapped, compact_str::format_compact!("row[{at}]").as_str());
            at += 1;
            let name = unique(&mut names, shared.field_name(&column.name));
    ...
    fn query(&mut self, query: &Query) -> Result<(), String> {
        ...
        for param in &query.params {
            ...
            let sql_name = if column.name.is_empty() {
                compact_str::format_compact!("column_{}", param.number).into_string()
            } else {
                column.name.clone()
            };
        ...
        let args_type = compact_str::format_compact!("{prefix}Args").into_string();
        ...
        let encode_param = |module: &mut Self, index: usize| -> String {
            let value = compact_str::format_compact!("args.{}", param_field[index]).into_string();
            if is_slice(index) {
                module.uses_sql = true;
                let codec = if param_mapped[index].unknown {
                    compact_str::format_compact!("{}.unknown", module.alias).into_string()
                } else {
                    module.codec(&param_mapped[index].codec)
                };
                compact_str::format_compact!("sql.slice({value}.map({codec}.encode))").into_string()
            } else {
        ...
        if is_copy {
            ...
            text_const = compact_str::format_compact!(
        ...
        let values_list = compact_str::format_compact!("[{}]", values.join(", ")).into_string();
        ...
        if rows {
            ...
            if single && (!single_nullable || matches!(cmd, ":many" | ":batchmany")) {
                ...
                decoder =
                    compact_str::format_compact!("(row) => {}", fields[0].decode).into_string();
            } else {
                if let Some(model) = self.whole_model(query) {
                    result_type = model.type_name.clone();
                    self.model_types.insert(model.type_name.clone());
                } else {
                    result_type = compact_str::format_compact!("{prefix}Row").into_string();
        ...
        let item_type = match cmd {
            ":one" | ":batchone" => {
                compact_str::format_compact!("{result_type} | null").into_string()
            }
            ":many" | ":batchmany" => {
                compact_str::format_compact!("Array<{result_type}>").into_string()
            }
        ...
        let body = if is_copy {
            ...
            let prelude = if prelude.is_empty() {
                ...
                compact_str::format_compact!("  {prelude}").into_string()
            };
            compact_str::format_compact!("{prelude}  return {};\n", call("db")).into_string()
        };
        let (signature, returns) = if is_copy {
            ...
            let returns = if cmd == ":batchexec" {
                ...
                compact_str::format_compact!("Array<{item_type}>").into_string()
            };
            (
            ...
            (
                compact_str::format_compact!("db: Queryable, args: {args_type}").into_string(),
                item_type,
            )
        } else {
