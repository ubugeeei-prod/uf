    ...
    for schema in &catalog.schemas {
        ...
        for entry in &schema.enums {
            ...
            enums.push(EnumInfo {
                ...
                values_name: format!("{base}Values"),
                codec_name: format!("{base}Codec"),
                type_name,
    ...
    for (filename, queries) in by_file {
        ...
        for query in queries {
            if !function_names.insert(query.name.clone()) {
                return Err(format!("two queries are called `{}`", query.name));
            }
            module
                .query(query)
                .map_err(|error| format!("{filename}: query {}: {error}", query.name))?;
        }
        ...
        files.push(File {
            name: format!("{stem}.js"),
            contents: module.finish(&format!("from `{stem}`")),
        });
    }
    Ok(files)
}
...
fn models_file(shared: &Shared<'_>) -> Result<String, String> {
    ...
    for model in &shared.models {
        ...
        let table_label = if model.schema == shared.default_schema {
            ...
            format!("{}.{}", model.schema, model.sql_name)
        };
        ...
        for column in &model.columns {
            ...
            fields.push(format!("{comment}  readonly {}: {flow},\n", key(&name)));
        }
        module
            .body
            .push_str(&format!("/** A row of `{table_label}`. */\n"));
        module.body.push_str(&format!(
            "export type {} = {{|\n{}|}};\n\n",
            model.type_name,
            fields.concat()
...
impl<'a> Module<'a> {
    ...
    fn slug(codec: &Codec) -> String {
        match codec {
            ...
            Codec::Array(inner) => format!("{}_array", Self::slug(inner)),
    ...
    fn hoist(&mut self, slug: &str, expression: String) -> String {
        ...
        let base = format!("{}Codec", camel(slug));
    ...
    fn codec(&mut self, codec: &Codec) -> String {
        ...
        match codec {
            Codec::Export(name) => format!("{alias}.{name}"),
            Codec::Factory(factory, sql_type) => self.hoist(
                sql_type,
                format!("{alias}.{factory}({})", js_string(sql_type)),
            ),
            Codec::Enum { codec, .. } => {
                if !self.is_models {
            ...
            Codec::Array(inner) => {
                ...
                self.hoist(&Self::slug(codec), format!("{alias}.array({inner_ref})"))
            }
            Codec::Mapped { base, over } => {
                ...
                self.hoist(
                    &over.name,
                    format!("{alias}.map({base_ref}, {}, {encode})", over.decode),
                )
            }
        }
    }
    ...
    fn decode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return format!("{alias}.unknown.decode({value})");
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            format!("{alias}.nullable({codec}, {value})")
        } else {
            format!("{codec}.decode({value})")
        }
    }
    ...
    fn encode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return format!("{alias}.unknown.encode({value})");
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            format!("{alias}.param({codec}, {value})")
        } else {
            format!("{codec}.encode({value})")
        }
    }
        ...
        for column in &query.columns {
            if let Some(embed) = &column.embed_table {
                let model = shared.model(embed).ok_or_else(|| {
                    format!("sqlc.embed({}) names no table in the schema", embed.name)
                })?;
                ...
                for table_column in &model.columns {
                    ...
                    let decode = self.decode(&mapped, &format!("row[{at}]"));
                    inner.push(format!("{}: {decode}", key(&name)));
                    at += 1;
                }
    ...
    fn query(&mut self, query: &Query) -> Result<(), String> {
        ...
        if !matches!(
            ...
            return Err(format!("the command `{cmd}` is not one sqlc defines"));
        }
        ...
        for param in &query.params {
                ...
                && let Some((existing, first)) = named.get(&sql_name)
            {
                ...
                if slice {
                    flow = format!("$ReadOnlyArray<{}>", mapped.flow);
                }
        ...
        let args_type = format!("{prefix}Args");
        if !arg_fields.is_empty() || is_batch || is_copy {
            self.claim(&args_type)?;
            let fields: String = arg_fields
                .iter()
                .map(|(name, flow)| format!("  readonly {}: {flow},\n", key(name)))
                .collect();
            self.body
                .push_str(&format!("export type {args_type} = {{|\n{fields}|}};\n\n"));
        }
        ...
        let encode_param = |module: &mut Self, index: usize| -> String {
            let value = format!("args.{}", param_field[index]);
            if is_slice(index) {
                ...
                format!("sql.slice({value}.map({codec}.encode))")
            } else {
        ...
        if is_copy {
            ...
            text_const = format!("const {sql_name} = {};\n\n", js_string(&query.text));
        } else {
            let found = placeholders::scan(&query.text, dialect);
            ...
            if expands {
                ...
                text_const.push_str(&format!("const {sql_name} = {};\n\n", js_string(&parts[0])));
            }
            for index in order {
        ...
        if rows {
            ...
            if single && (!single_nullable || matches!(cmd, ":many" | ":batchmany")) {
                ...
                decoder = format!("(row) => {}", fields[0].decode);
            } else {
                if let Some(model) = self.whole_model(query) {
                    result_type = model.type_name.clone();
                    self.model_types.insert(model.type_name.clone());
                } else {
                    result_type = format!("{prefix}Row");
                    self.claim(&result_type)?;
                    let lines: String = fields
                        .iter()
                        .map(|field| format!("  readonly {}: {},\n", key(&field.name), field.flow))
                        .collect();
                    self.body
                        .push_str(&format!("export type {result_type} = {{|\n{lines}|}};\n\n"));
                }
                let entries: Vec<String> = fields
                    .iter()
                    .map(|field| format!("{}: {}", key(&field.name), field.decode))
                    .collect();
                decoder = format!("(row) => ({{ {} }})", entries.join(", "));
            }
        }
        ...
        let (text_expr, params_expr, prelude) = if expands {
            (
                "text".to_owned(),
                "params".to_owned(),
                format!("const {{ text, params }} = sql.expand({sql_name}, {values_list});\n"),
            )
        } else {
        ...
        let call = |db: &str| -> String {
            match cmd {
                ":one" | ":batchone" => {
                    format!("sql.one({db}, {name}, {text_expr}, {params_expr}, {width}, {decoder})")
                }
                ":many" | ":batchmany" => {
                    format!(
                ...
                ":execrows" => format!("sql.execRows({db}, {name}, {text_expr}, {params_expr})"),
                ":execresult" => {
                    format!("sql.execResult({db}, {name}, {text_expr}, {params_expr})")
                }
                ":execlastid" => {
                    format!("sql.execLastId({db}, {name}, {text_expr}, {params_expr})")
                }
                _ => format!("sql.exec({db}, {name}, {text_expr}, {params_expr})"),
            }
        };
        let item_type = match cmd {
            ":one" | ":batchone" => format!("{result_type} | null"),
            ":many" | ":batchmany" => format!("Array<{result_type}>"),
        ...
        let body = if is_copy {
            ...
            format!("  return sql.batch(db, items, (q, args) => {inner}){then};\n")
        } else {
            ...
            format!("{prelude}  return {};\n", call("db"))
        };
        let (signature, returns) = if is_copy {
            (
                format!("db: Queryable, rows: $ReadOnlyArray<{args_type}>"),
                item_type,
            )
        } else if is_batch {
            ...
            (
                format!("db: Queryable, items: $ReadOnlyArray<{args_type}>"),
                returns,
            )
        } else if has_args {
            (format!("db: Queryable, args: {args_type}"), item_type)
        } else {
    ...
    fn finish(mut self, what: &str) -> String {
        ...
        for (name, expression) in &self.hoisted {
            rest.push_str(&format!("const {name} = {expression};\n"));
        }
        ...
        if self.uses_sql {
            out.push_str(&format!("import * as sql from {};\n", js_string(runtime)));
        }
        if self.uses_codecs {
            out.push_str(&format!(
                "import * as {} from {};\n",
                self.alias,
                js_string(&format!("{runtime}/{}", shared.engine.module()))
            ));
        }
        ...
        out.push('\n');
        for (name, expression) in &self.hoisted {
            out.push_str(&format!("const {name} = {expression};\n"));
        }
