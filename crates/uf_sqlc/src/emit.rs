...
pub fn generate(request: &GenerateRequest) -> Result<Vec<File>, String> {
    ...
    let qualify = |schema: &str, name: &str| {
        if schema == default_schema || schema.is_empty() {
            ...
            uf_infra::into_string(compact_str::format_compact!("{schema}_{name}"))
        }
    };
    ...
    for schema in &catalog.schemas {
        ...
        for entry in &schema.enums {
            ...
            enums.push(EnumInfo {
                ...
                values_name: uf_infra::into_string(compact_str::format_compact!("{base}Values")),
                codec_name: uf_infra::into_string(compact_str::format_compact!("{base}Codec")),
                type_name,
    ...
    for (filename, queries) in by_file {
        ...
        files.push(File {
            name: uf_infra::into_string(compact_str::format_compact!("{stem}.js")),
...
fn doc(lines: &[String], indent: &str) -> String {
    ...
    let mut out = uf_infra::into_string(compact_str::format_compact!("{indent}/**\n"));
    for line in lines {
...
fn header(shared: &Shared<'_>, what: &str) -> String {
    ...
    let with = if version.is_empty() {
        ...
        uf_infra::into_string(compact_str::format_compact!(" with sqlc {version}"))
    };
    uf_infra::into_string(compact_str::format_compact!(
...
fn unique(names: &mut BTreeSet<String>, name: String) -> String {
    ...
    for n in 2.. {
        let candidate = uf_infra::into_string(compact_str::format_compact!("{name}{n}"));
...
impl<'a> Module<'a> {
    ...
    fn slug(codec: &Codec) -> String {
        match codec {
            ...
            Codec::Array(inner) => {
                uf_infra::into_string(compact_str::format_compact!("{}_array", Self::slug(inner)))
            }
    ...
    fn hoist(&mut self, slug: &str, expression: String) -> String {
        ...
        let base = uf_infra::into_string(compact_str::format_compact!("{}Codec", camel(slug)));
        let mut name = base.clone();
        let mut n = 2;
        while self.taken.contains(&name) {
            name = uf_infra::into_string(compact_str::format_compact!("{base}{n}"));
            n += 1;
        }
    ...
    fn codec(&mut self, codec: &Codec) -> String {
        ...
        match codec {
            Codec::Export(name) => {
                uf_infra::into_string(compact_str::format_compact!("{alias}.{name}"))
            }
    ...
    fn decode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return uf_infra::into_string(compact_str::format_compact!(
                "{alias}.unknown.decode({value})"
            ));
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            uf_infra::into_string(compact_str::format_compact!(
                "{alias}.nullable({codec}, {value})"
            ))
        } else {
            uf_infra::into_string(compact_str::format_compact!("{codec}.decode({value})"))
        }
    }
    ...
    fn encode(&mut self, mapped: &Mapped, value: &str) -> String {
        ...
        if mapped.unknown {
            self.uses_codecs = true;
            return uf_infra::into_string(compact_str::format_compact!(
                "{alias}.unknown.encode({value})"
            ));
        }
        let codec = self.codec(&mapped.codec);
        if mapped.nullable {
            uf_infra::into_string(compact_str::format_compact!(
                "{alias}.param({codec}, {value})"
            ))
        } else {
            uf_infra::into_string(compact_str::format_compact!("{codec}.encode({value})"))
        }
    }
        ...
        for column in &query.columns {
            if let Some(embed) = &column.embed_table {
                ...
                for table_column in &model.columns {
                    ...
                    inner.push(uf_infra::into_string(compact_str::format_compact!(
                        "{}: {decode}",
                        key(&name)
                    )));
                    at += 1;
    ...
    fn query(&mut self, query: &Query) -> Result<(), String> {
        ...
        for param in &query.params {
            ...
            let sql_name = if column.name.is_empty() {
                uf_infra::into_string(compact_str::format_compact!("column_{}", param.number))
            } else {
                column.name.clone()
            };
        ...
        let args_type = uf_infra::into_string(compact_str::format_compact!("{prefix}Args"));
        ...
        let encode_param = |module: &mut Self, index: usize| -> String {
            let value =
                uf_infra::into_string(compact_str::format_compact!("args.{}", param_field[index]));
            if is_slice(index) {
                module.uses_sql = true;
                let codec = if param_mapped[index].unknown {
                    uf_infra::into_string(compact_str::format_compact!("{}.unknown", module.alias))
                } else {
                    module.codec(&param_mapped[index].codec)
                };
                uf_infra::into_string(compact_str::format_compact!(
                    "sql.slice({value}.map({codec}.encode))"
                ))
            } else {
        ...
        let values_list =
            uf_infra::into_string(compact_str::format_compact!("[{}]", values.join(", ")));
        ...
        if rows {
            ...
            if single && (!single_nullable || matches!(cmd, ":many" | ":batchmany")) {
                ...
                if let Some(model) = self.whole_model(query) {
                    ...
                    result_type =
                        uf_infra::into_string(compact_str::format_compact!("{prefix}Row"));
        ...
        let item_type = match cmd {
            ":one" | ":batchone" => {
                uf_infra::into_string(compact_str::format_compact!("{result_type} | null"))
            }
            ":many" | ":batchmany" => {
                uf_infra::into_string(compact_str::format_compact!("Array<{result_type}>"))
            }
        ...
        let body = if is_copy {
            ...
            let prelude = if prelude.is_empty() {
                ...
                uf_infra::into_string(compact_str::format_compact!("  {prelude}"))
            };
            uf_infra::into_string(compact_str::format_compact!(
                "{prelude}  return {};\n",
                call("db")
