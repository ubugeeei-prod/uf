...
//!
//! Two of them are not about the dialect at all: which `exports` conditions a
//! package is resolved under, and which field enters one that has no `exports`
//! map. Those decide *which file* the checker types rather than how it types
//! it, they are a claim about every JavaScript host uf supports, and
//! [`crate::resolution`] is where they live and where the claim is argued.
...
mod tests {
    ...
    /// The resolution decision reaches Flow intact.
    ///
    /// [`crate::resolution`] is where the conditions are argued and where the
    /// tests about *which* ones they are live; this one is only that nothing
    /// between there and Flow's `Options` drops or reorders them, because a
    /// condition set that is correct and unread is the same as a wrong one.
    #[test]
