...
//! bound on disk, never on correctness — and it is the *inner* of two, since
//! [`CheckCache::sweep`] bounds the directory whatever any one record does. So
//! the worst a project checked many ways costs is four answers per live record
//! rather than an unbounded number, inside a total that was already fixed.
//!
//! # What is on disk
//!
