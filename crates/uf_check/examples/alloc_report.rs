...
fn cache_report(sources: &[Source<'_>]) {
    ...
    // cost less.
    let edited_source = uf_infra::cstr!("{}\n", sources[0].source).into_string();
...
fn copy_path(path: &str, index: usize) -> String {
    ...
    let name = match extension {
        Some(extension) => uf_infra::cstr!("{stem}.copy{index}.{extension}").into_string(),
        None => uf_infra::cstr!("{stem}.copy{index}").into_string(),
    };
