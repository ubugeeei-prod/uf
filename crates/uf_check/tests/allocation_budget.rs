...
//!
//! The issue was filed from `packages/router/internal/runtime.js`, where a cold
...
fn runtime_js_check_stays_below_the_cold_allocation_budget() {
    ...
    let sources = [Source::new("packages/router/internal/runtime.js", &source)];
    ...
    assert!(
        ...
        "checking {} bytes of router runtime took {} allocations, over the \
         ...
         -p uf_check -- packages/router/internal/runtime.js --phases` to find the phase.",
        source.len(),
        delta.allocations
    );
    assert!(
