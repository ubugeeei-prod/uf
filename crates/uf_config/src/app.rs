...
impl TrailingSlash {
    ...
    pub fn spell(self, path: &str) -> String {
        ...
        match self {
            Self::Always => uf_infra::into_string(compact_str::format_compact!("{trimmed}/")),
...
pub enum RenderingMode {
    ...
    ///
    /// A prerendered page that states a lifetime with `cacheLife` is served
    /// from the build and regenerated by the server once the lifetime passes,
    /// through the route cache — so it takes `rendering.cache.route` on and a
    /// deployment with a server. A page that states none is a static document,
    /// which is why allowing `isr` allows what `ssg` does. See
    /// ubugeeei-prod/uf#950.
    Isr,
