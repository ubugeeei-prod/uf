...
fn the_step_is_the_component_that_changed() {
    assert_eq!(step(&version("1.2.3"), &version("1.2.4")), Some(Level::Patch));
    assert_eq!(step(&version("1.2.3"), &version("1.3.0")), Some(Level::Minor));
    assert_eq!(step(&version("1.2.3"), &version("2.0.0")), Some(Level::Major));
...
fn best_is_capped_by_the_level_that_was_asked_for() {
    ...
    assert_eq!(best(&published, &from, Level::Patch), Some(&version("1.2.9")));
    assert_eq!(best(&published, &from, Level::Minor), Some(&version("1.4.0")));
    assert_eq!(best(&published, &from, Level::Major), Some(&version("3.1.0")));
}
