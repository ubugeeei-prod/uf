// @flow
component Empty() {}
component Simple(name: string, count?: number = 0) renders Node {
  return <p>{name}</p>;
}
component WithRest(name: string, ...rest: Props) {
  return null;
}
component Renamed("data-id" as dataId: string, className as cls: string = "x") {}
component Generic<T>(value: T) renders? Item<T> {
  return null;
}
component RendersStar(items: Array<Item>) renders* Item {
  return items;
}
export component Exported(a: string) {
  return a;
}
export default component DefaultExported() {
  return null;
}
async component AsyncComponent() {
  return null;
}
component LongParams(
  firstParameter: FirstType,
  secondParameter: SecondType,
  thirdParameter: ThirdType,
) {}
hook useNothing() {}
hook useCounter(initial: number): [number, () => void] {
  const [count, setCount] = useState(initial);
  return [count, () => setCount(count + 1)];
}
export hook useExported<T>(value: T): T {
  return value;
}
hook useLongParameters(
  firstParameter: FirstType,
  secondParameter: SecondType,
  third: ThirdType,
): Result {}
