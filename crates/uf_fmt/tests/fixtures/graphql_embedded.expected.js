...
`;
const byShortTag = gql`
  query Q {
    field
  }
`;
const byExperimental = graphql.experimental`
  query Q {
    field
  }
`;
const byComment = /* GraphQL */ `
  query Q {
    field
  }
`;
...
const directives = graphql`
  fragment F on Persona {
    basicUser {
      ...RelayClient3DModuleTestFragmentClientUser_data_withALongEnoughNameToBreak
...
const query = graphql`
  query LongOperationNameQuery(
    $firstVariable: String!
    $secondVariable: [Int!]! = [1, 2]
    $third: Boolean
  ) @dir {
...
`;
const anonymousWithVariables = graphql`
  query ($x: Int) {
    f(a: $x)
  }
`;
