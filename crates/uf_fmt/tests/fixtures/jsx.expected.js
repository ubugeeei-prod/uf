...
const mixedText = (
  <p>
    Hello, {name}! You have {count} new {count === 1 ? "message" : "messages"}.
  </p>
);
const longText = (
  <p>
    This is a very long paragraph of text that will need to be wrapped across multiple lines by the
    formatter because it is long.
  </p>
);
...
const conditionalLong = someCondition ? (
  <FirstComponent withProp={value} />
) : (
  <SecondComponent withOtherProp={other} />
);
...
function Component() {
  return (
    <div className="wrapper">
      <Header title={title} subtitle={subtitle} onClick={handleClick} />
      <Body>{children}</Body>
    </div>
  );
}
...
const veryLongAttributeList = (
  <Component firstProperty={firstValue} secondProperty={secondValue} thirdProperty={thirdValue} />
);
const parenthesized = <div>text</div>;
const returned = () => (
  <div>
    <span />
  </div>
...
const textAroundElements = (
  <p>
    before <b>bold</b> after
  </p>
);
const selfClosingText = (
  <p>
    text <br /> more
  </p>
);
