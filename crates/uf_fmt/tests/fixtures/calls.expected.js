...
callWithFunction(function named() {
  return 1;
}, second);
promise
...
const memoized = useCallback((event) => {
  setValue(event.target.value);
}, []);
describe("suite", () => {
  it("does something really quite long in the description text here", async () => {
    await run();
  });
});
...
fn(a)(b)(c);
curried(argumentOne)(argumentTwo)(argumentThree)(argumentFour)(argumentFive)(argumentSix)(
  argumentSeven,
);
...
const longFirst = someObject
  .someMethodWithALongName(argumentNumberOne, argumentNumberTwo)
  .anotherMethod();
...
async function fetching() {
  const value = await fetchSomething(url, {
    method: "POST",
...
foo({ a: 1, b: 2 }, [1, 2, 3]);
foo([1, 2, 3], { a: 1, b: 2 });
veryLongFunctionName(anotherLongArgumentName, {
  key: value,
  another: thing,
  more: stuff,
  evenMore: 1,
...
veryLongFunctionName(
  { key: value, another: thing, more: stuff, evenMore: 1 },
