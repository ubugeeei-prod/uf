...
const nodeAlreadyExistsInConnection = connection.getLinkedRecords(EDGES)?.some((edge) => edge?.getLinkedRecord(NODE)?.getDataID() === serverNodeId);

// The same chain without the `?.`, for comparison.
const two = connection.getLinkedRecords(EDGES).some((edge) => edge?.getLinkedRecord(NODE)?.getDataID() === serverNodeIds);
