...
function longParamsWithDefaults(
  firstParameterName = 1,
  secondParameterName = 2,
  thirdParameterName = 3,
) {}
function typedLongParams(
  first: FirstType,
  second: SecondType,
  third: ThirdType,
  fourth: FourthType,
...
const arrowBodyCall = (x) =>
  someFunction(x, withAnotherArgument, andYetAnotherOne, andOneMoreToMakeItLong);
const arrowBodyObject = (x) => ({
  key: x,
  another: 1,
  more: 2,
  evenMore: 3,
...
const arrowBodyTernary = (x) =>
  x ? somethingQuiteLongToReturnWhenTrue : somethingQuiteLongToReturnWhenFalse;
const arrowBodyBinary = (x) =>
  x.someProperty + x.anotherProperty + x.yetAnotherProperty + x.oneMoreProperty + x.last;
...
const arrowChainCallee = (
  (a) => (b) =>
    a + b
)(1)(2);
const assignedArrowChain = (firstArgument) => (secondArgument) => (thirdArgument) =>
  firstArgument + secondArgument + thirdArgument + 1;
promise.then(
  (result) => (result.ok ? result.value : defaultValueThatIsQuiteLongIndeed) + anotherValueToAdd,
);
