...
const typeDefs = gql`
  schema @dir { query: MyQuery, mutation: MyMutation
  ...
  type User implements Node & Actor @key(fields: "id") {
    "The identifier." id: ID!
    name(first: Int = 10, "How to page." after: String): [String!]!

    friends: [User] @deprecated(reason: "use connections")
    connections(first: Int, after: String, orderBy: [OrderBy!], includeHiddenOnes: Boolean): [User!]!
  }
  extend type User @patch { extra: Int }

  interface Node { id: ID! }

  union Short = A | B
  union LongUnionName = AVeryLongMemberNameHere | AnotherVeryLongMemberName | AThirdLongMemberNameHere
  ...
  input Filter {
    "By name." name: String = "unnamed"
    """
    By age.
    """
    age: Int! = 3 @dir
  }
  extend input Filter { extra: [Int!]! = [1, 2, 3] }

  directive @foo(a: Int = 1, b: String) repeatable on FIELD | FRAGMENT_SPREAD | INLINE_FRAGMENT
  extend directive @foo @bar

  extend schema @link(url: "https://spec") { query: OtherQuery }
`;
