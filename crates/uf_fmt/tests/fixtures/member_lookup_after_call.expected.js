...
function DetachArrayBuffer(realm: Realm, arrayBuffer: ObjectValue) {
  ...
  let block = Properties.ThrowIfInternalSlotNotWritable(
    realm,
    arrayBuffer,
    "$ArrayBufferData",
  ).$ArrayBufferData;
...
let a2 = fn(
  ...
  bbbbbbbbbbbbbbbbbbbbbbbbbbbbXYZ,
).someVeryLongPropertyNameHereOk;

// A member between the call and the `=` is what stands to break, so it does,
// and the tail is not glued to it.
let a4 = obj.fn(aaaaaaaaaaaaaaaaaaaaaaaa, bbbbbbbbbbbbbbbbbbbbbbbbXYZ).prop
  .someVeryLongPropertyName;
...
let c2 = object
  .methodOne(argument)
  .methodTwo(argument)
  .methodThree(argument).someLongPropertyNameXY;

// And the same chain with one more lookup in between.
let d1 = object.methodOne(argument).methodTwo(argument).methodThree(argument)
  .prop.someLongPropXYZWV;
