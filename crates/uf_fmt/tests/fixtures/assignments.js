// @flow
const shortValue = 1;
const aVeryLongVariableNameThatTakesUpSpace = anotherVeryLongFunctionName(withSomeArguments, andMore);
const chained = one.two.three.four.five.six.seven.eight.nine.ten.eleven.twelve.thirteen.fourteen;
const stringValue = "a very long string literal that does not fit on the line with the declaration at all";
...
const conditional = someVeryLongConditionExpression ? consequentValueWhenTrue : alternateValueWhenFalse;
const conditionalWithBinaryTest = firstOperand && secondOperand ? consequentValueWhenTrue : alternateValue;
const classValue = class extends SomeVeryLongBaseClassName { method() { return 1; } };
...
const nested = { outer: { inner: { deep: { deeper: { deepest: "a value that pushes this past" } } } } };
const destructured = { alpha, beta, gamma, delta, epsilon, zeta, eta, theta, iota, kappa } = source;
const { alpha: renamedAlpha, beta: renamedBeta, gamma: renamedGamma = defaultValue } = anotherSource;
const [firstElementName, secondElementName, thirdElementName, fourthElementName] = tupleSource;
const typed: SomeVeryLongTypeName<WithTypeArguments, AndMore> = someFunctionCall(withArguments, more);
...
exports.handler = async function handler(event, context) { return { statusCode: 200, body: "" }; };
...
const shortKeyObject = { a: someVeryLongFunctionCallExpression(withArguments, andMoreArguments, andMore) };
const longKeyObject = { aReasonablyLongKey: someVeryLongFunctionCallExpression(withArguments, andMore) };
const regexValue = /a very long regular expression pattern that goes on and on and on and on and on/gi;
let uninitialized;
let multiple = 1, declarations = 2, here = 3;
let multipleLong = someLongInitializerExpression(withArguments), another = anotherInitializer(more);
