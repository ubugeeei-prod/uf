...
export component ItemList(title: string, items: $ReadOnlyArray<{| +id: string, +label: string |}>, onSelect?: (id: string) => void) renders Node {
  ...
  return (
    <section className="item-list" aria-label={title}>
      ...
      <ul>
        {visible.map((item) => (
          <li key={item.id} className={item.id === selected ? "selected" : undefined} onClick={() => setSelected(item.id)}>
            {item.label}
          </li>
        ))}
      </ul>
