...
type Obj = { a: string, b?: number, +c: boolean, -d: string, readonly e: number, [key: string]: mixed, (): void, [[slot]]: T, m(): void, ...Spread, ... };
...
type LongUnion = "first-option" | "second-option" | "third-option" | "fourth-option" | "fifth-option-here";
type ObjectUnion = { kind: "a", value: string } | { kind: "b", value: number } | { kind: "c", value: boolean };
...
type Existential = *;
type LongGeneric = SomeGenericType<FirstTypeArgument, SecondTypeArgument, ThirdTypeArgument, FourthTypeArgument>;
type LongFunction = (firstParameter: FirstType, secondParameter: SecondType, thirdParameter: ThirdType) => ReturnType;
type LongObject = { firstProperty: FirstType, secondProperty: SecondType, thirdProperty: ThirdType, fourth: Fourth };
...
interface Iface { a: string; b(): void; +c: number }
interface Extended extends Base, Other { extra: string }
interface WithQualified extends Module.Base {}
declare interface DeclaredIface { x: number }
...
function predicate(x: mixed): boolean %checks { return typeof x === "string"; }
function guard(x: mixed): implies x is string { return true; }
function assertsGuard(x: mixed): asserts x is string {}
const typedArrow = (x: number): string => String(x);
const genericArrow = <T>(x: T): T => x;
const genericArrowLong = <TypeParameterOne, TypeParameterTwo>(argument: TypeParameterOne): TypeParameterTwo => convert(argument);
let annotatedLet: Array<{ a: string, b: number }> = [];
const objectFn = { fn(x: string): number { return 1; } };
type CallableObject = { (x: number): string, prop: boolean };
type LongUnionInGeneric = Array<"first-option" | "second-option" | "third-option" | "fourth-option" | "fifth">;
type FunctionReturningUnion = () => "first-option" | "second-option" | "third-option" | "fourth-option" | "x";
type Nested = { a: { b: { c: string } } };
type LongIntersection = FirstLongTypeName & SecondLongTypeName & ThirdLongTypeName & FourthLongTypeName;
