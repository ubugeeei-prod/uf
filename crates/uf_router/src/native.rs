...
pub fn discover_route_table(
    ...
    if app_root.is_dir() {
        let not_found_stem =
            compact_str::format_compact!("${}", ReservedRole::NotFound.as_str()).into_string();
        let error_stem =
            compact_str::format_compact!("${}", ReservedRole::Error.as_str()).into_string();
...
pub fn native_router_module(
    ...
    manifest.with_file_name(
        compact_str::format_compact!("{stem}.{}.{extension}", platform.as_str()).into_string(),
    )
}
...
pub fn generate_native_router_flow(
    ...
    let layout_ids = |chain: &[Utf8PathBuf]| {
        chain
            ...
            .map(|index| compact_str::format_compact!("layout{index}").into_string())
            .collect::<Vec<_>>()
            .join(", ")
    };
    ...
    let mut output = compact_str::format_compact!(
    ...
    for (index, layout) in layouts.iter().enumerate() {
        let segment = layout
            ...
            .map_or_else(
                || "/".to_string(),
                |relative| {
                    compact_str::format_compact!("/{}", slash_joined(relative)).into_string()
                },
            );
...
fn import_specifier(directory: &Utf8Path, file: &Utf8Path) -> String {
    ...
    if joined.starts_with("..") {
        ...
        compact_str::format_compact!("./{joined}").into_string()
    }
}
