...
pub fn discover_route_table(
    ...
    if app_root.is_dir() {
        let not_found_stem = uf_infra::into_string(compact_str::format_compact!(
            "${}",
            ReservedRole::NotFound.as_str()
        ));
        let error_stem = uf_infra::into_string(compact_str::format_compact!(
            "${}",
            ReservedRole::Error.as_str()
        ));
...
pub fn native_router_module(
    ...
    manifest.with_file_name(uf_infra::into_string(compact_str::format_compact!(
        "{stem}.{}.{extension}",
        platform.as_str()
    )))
}
...
pub fn generate_native_router_flow(
    ...
    let layout_ids = |chain: &[Utf8PathBuf]| {
        chain
            ...
            .map(|index| uf_infra::into_string(compact_str::format_compact!("layout{index}")))
            .collect::<Vec<_>>()
            .join(", ")
    };
    ...
    let mut output = uf_infra::into_string(compact_str::format_compact!(
