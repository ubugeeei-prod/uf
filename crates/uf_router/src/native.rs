...
pub fn discover_route_table(
    ...
    if app_root.is_dir() {
        let not_found_stem =
            uf_infra::into_string(uf_infra::cstr!("${}", ReservedRole::NotFound.as_str()));
        let error_stem =
            uf_infra::into_string(uf_infra::cstr!("${}", ReservedRole::Error.as_str()));
...
pub fn native_router_module(
    ...
    manifest.with_file_name(uf_infra::into_string(uf_infra::cstr!(
        "{stem}.{}.{extension}",
        platform.as_str()
    )))
}
...
pub fn generate_native_router_flow(
    ...
    let layout_ids = |chain: &[Utf8PathBuf]| {
        chain
            ...
            .map(|index| uf_infra::into_string(uf_infra::cstr!("layout{index}")))
            .collect::<Vec<_>>()
            .join(", ")
    };
    ...
    let mut output = uf_infra::into_string(uf_infra::cstr!(
    ...
    for route in &table.routes {
        ...
        let _ = write!(
            ...
            matches!(route.route.page.extension(), Some("mdx" | "md")),
    ...
    for boundary in &table.not_found {
        let _ = write!(
            ...
            matches!(boundary.file.extension(), Some("mdx" | "md")),
    ...
    for (index, layout) in layouts.iter().enumerate() {
        let segment = layout
            ...
            .map_or_else(
                || "/".to_string(),
                |relative| uf_infra::into_string(uf_infra::cstr!("/{}", slash_joined(relative))),
            );
