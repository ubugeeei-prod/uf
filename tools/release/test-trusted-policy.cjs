...
const workflow = fs.readFileSync(
	path.resolve(__dirname, "../../.github/workflows/release-policy.yml"),
	"utf8",
);
...
function check({
	...
	const event =
		queueBase === undefined
			...
			: {
					merge_group: {
						head_ref: `refs/heads/gh-readonly-queue/main/pr-7-${head}`,
						base_sha: queueBase,
					},
				};
	const pr = {
		...
		head: {
			ref: branch,
			sha: head,
			repo: { full_name: fork ? "fork/project" : repo },
		},
	...
	const response = (route) => {
		...
		if (route.includes("/git/trees/"))
			return {
				tree: [
					{
						path: migrated && route.endsWith(head) ? "npm" : "packages",
						type: "tree",
					},
				],
			};
	...
	vm.runInNewContext(source, {
		...
		process: {
			env: { GITHUB_REPOSITORY: repo, GITHUB_EVENT_PATH: "event.json" },
		},
		console: { log() {} },
		Buffer,
	});
}
test("trusted workflow accepts a maintainer release on current main", () =>
	check());
test("ordinary PRs do not require release permissions", () =>
	check({
		role: "write",
		branch: "fix/typo",
		files: ["crates/uf_cli/src/lib.rs"],
	}));
	...
	assert.throws(
		() =>
			check({
				role: "write",
				branch: "fix/policy",
				files: [".github/workflows/release-policy.yml"],
			}),
		/maintain or admin/,
	);
	assert.throws(
		() =>
			check({
				role: "write",
				branch: "fix/editors",
				files: [".github/workflows/editors.yml"],
			}),
		/maintain or admin/,
	);
});
...
test("workspace renames preserve the version and release authorization checks", () => {
	check({
		role: "write",
		branch: "refactor/npm-layout",
		files: ["packages/core/package.json", "npm/core/package.json"],
		migrated: true,
	});
	assert.throws(
		() =>
