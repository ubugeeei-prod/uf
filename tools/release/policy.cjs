...
function assertMaintainer(permission /*: Permission */) /*: void */ {
	if (
		...
		throw new Error(
			"Releases require repository maintain or admin permission.",
		);
	}
}
...
function assertValidation(
	...
	if (
		...
		throw new Error(
			"Release requires successful merge-queue CI for the exact merged commit.",
		);
	}
}
...
function requestAt(ref /*: string */, io /*: IO */ = IO) /*: ReleaseRequest */ {
	const request /*: ReleaseRequest */ = JSON.parse(
		io.git("show", `${ref}:.github/release.json`),
	);
...
function releasePR(
	...
	if (prs.length !== 1)
		throw new Error("Expected exactly one release PR for this version.");
	const pr /*: PullRequest */ = io.api(
		`repos/${repository}/pulls/${prs[0].number}`,
	);
	assertPullRequest(pr, request, repository);
	assertMaintainer(
		io.api(
			`repos/${repository}/collaborators/${encodeURIComponent(pr.user.login)}/permission`,
		),
	);
	return pr;
}
function checkCandidate(
	...
	if (!requested || previous === current)
		throw new Error(
			"A version bump and release request must be committed together.",
		);
	const request = requestAt("HEAD");
	const repository = requireEnv(process.env, "GITHUB_REPOSITORY");
	const event = JSON.parse(
		fs.readFileSync(requireEnv(process.env, "GITHUB_EVENT_PATH"), "utf8"),
	);
...
function assertQueueTree(
	...
	if (
		git("rev-parse", `${head}^{tree}`) !== git("rev-parse", `${queued}^{tree}`)
	)
		throw new Error(
			"The release merge group must contain exactly the current release PR tree.",
		);
}
function assertArtifacts(
	...
	for (const target of [
		...
		const artifact = artifacts.find(
			(item) => item.name === `uf-release-${target}`,
		);
...
function assertTagTarget(
	...
	const found /*: ?{ readonly ref: string, readonly object: GitObject, ... } */ =
		io
			.api(
	...
	if (object.type !== "commit" || object.sha !== commit)
		throw new Error(
			"The release tag already points to another commit; it will not be replaced.",
		);
}
/**
...
function authorizePublication(
	...
	assertValidation(
		io.api(`repos/${repository}/actions/runs/${runId}`),
		commit,
		repository,
	);
	assertArtifacts(
		io.api(`repos/${repository}/actions/runs/${runId}/artifacts?per_page=100`)
			.artifacts,
	);
	if (env.NPM_RUN) {
		...
		if (
			...
			throw new Error(
				"The npm publication and verification must succeed first.",
			);
	}
	const who =
		via === "automation" ? "release automation" : String(env.GITHUB_ACTOR);
	console.log(
		`Authorized ${version} from PR #${pr.number} at ${commit} (dispatched by ${who})`,
	);
