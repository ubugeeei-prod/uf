...
const { findRelease, autoRelease, runWorkflow, VERIFY_JOB } = require("./auto-release.cjs");
...
function world(patch = {}) {
  ...
  const api = (route) => {
    ...
    if (permission)
      return { role_name: facts.roles[decodeURIComponent(permission[1])] ?? "read" };
    if (route.includes("/git/matching-refs/tags/")) return [];
    if (route.endsWith("/git/ref/heads/main")) return { object: { sha: commit } };
...
test("a maintainer's manual dispatch is authorized as before", () => {
  assert.equal(quiet(() => authorizePublication(env(), world())).via, "maintainer");
  assert.equal(
    quiet(() => authorizePublication(env({ GITHUB_ACTOR: "admin" }), world())).via,
    "maintainer",
  );
});
...
test("automation is authorized by the release PR, not by its own permission", () => {
  const result = quiet(() => authorizePublication(env({ GITHUB_ACTOR: AUTOMATION_ACTOR }), world()));
  assert.deepEqual(result, { pr: 7, via: "automation" });
});
test("automation cannot publish a release PR a non-maintainer authored", () => {
  assert.throws(
    () => authorizePublication(env({ GITHUB_ACTOR: AUTOMATION_ACTOR }), world({ author: "someone" })),
    /maintain or admin/,
  );
});
test("automation cannot publish an unmerged PR, or one merged at another commit", () => {
  const bot = env({ GITHUB_ACTOR: AUTOMATION_ACTOR });
  assert.throws(() => authorizePublication(bot, world({ merged: false })), /not merged at this commit/);
  assert.throws(
...
test("automation cannot publish without the queue's validation of the commit", () => {
  const bot = env({ GITHUB_ACTOR: AUTOMATION_ACTOR });
  assert.throws(() => authorizePublication(bot, world({ validation: null })), /404/);
  const failed = { ...world().api(`repos/${repository}/actions/runs/100`), conclusion: "failure" };
  assert.throws(
...
test("publication is refused off main, whoever dispatches it", () => {
  for (const actor of ["maintainer", AUTOMATION_ACTOR])
    assert.throws(
      () => authorizePublication(env({ GITHUB_ACTOR: actor, GITHUB_REF: "refs/heads/x" }), world()),
      /from main/,
    );
});
...
test("a commit whose release PR is unmerged, or merged elsewhere, is not a release", () => {
  ...
  assert.equal(findRelease(commit, repository, world({ mergeCommit: "d".repeat(40) })), null);
});
...
function actions(script = {}) {
  ...
  const base = world();
  const api = (route) => {
    const listed = /actions\/workflows\/([a-z]+\.yml)\/runs\?event=workflow_dispatch/.exec(route);
    if (listed)
      return { workflow_runs: runs.filter((run) => run.workflow === listed[1]).map(view) };
    ...
    if (one && Number(one[1]) >= 200) return view(runs.find((run) => run.id === Number(one[1])));
  ...
  const post = (route, body) => {
    ...
    const dispatch = /actions\/workflows\/([a-z]+\.yml)\/dispatches$/.exec(route);
    if (dispatch) {
      const attempts = (script[dispatch[1]] || [{ conclusion: "success", jobs: [] }]).slice();
      runs.push({
        id: id++,
        workflow: dispatch[1],
    ...
    if (rerun) {
      ...
      run.attempts.push(run.next.shift() || { conclusion: "success", jobs: [] });
      return;
    }
...
test("a release commit dispatches publish, release and editors in order", async () => {
  ...
  const result = await autoRelease({ GITHUB_REPOSITORY: repository, RELEASE_COMMIT: commit }, io);
  assert.deepEqual(result, { version, pr: 7 });
  assert.deepEqual(
    posted.map((item) => item.route.split("/").slice(-2).join("/")),
    ["publish.yml/dispatches", "release.yml/dispatches", "editors.yml/dispatches"],
  );
...
test("a commit that is not a release dispatches nothing", async () => {
  const { io, posted } = actions();
  const other = { ...io, git: world({ previousRequest: { version, branch } }).git };
  assert.equal(await autoRelease({ GITHUB_REPOSITORY: repository, RELEASE_COMMIT: commit }, other), null);
  assert.deepEqual(posted, []);
});
test("a release with no successful queue run dispatches nothing and fails", async () => {
  const { io, posted } = actions();
  const bare = world({ validation: null });
  const api = (route) => (route.includes("ci.yml/runs") ? bare.api(route) : io.api(route));
  await assert.rejects(
    autoRelease({ GITHUB_REPOSITORY: repository, RELEASE_COMMIT: commit }, { ...io, api }),
    /No successful merge-queue CI run/,
  );
  assert.deepEqual(posted, []);
});
test("a lagging npm verification is rerun once, and then the release goes on", async () => {
  const { io, posted } = actions({ "publish.yml": [verifyFailed, { conclusion: "success", jobs: [] }] });
...
test("any other failure stops the release without a rerun", async () => {
  ...
  const released = { conclusion: "failure", jobs: [{ name: VERIFY_JOB, conclusion: "failure" }] };
  const second = actions({ "release.yml": [released] });
  await assert.rejects(
    autoRelease({ GITHUB_REPOSITORY: repository, RELEASE_COMMIT: commit }, second.io),
    /Release 0\.3\.0/,
  );
  assert.ok(!second.posted.some((item) => item.route.endsWith("rerun-failed-jobs")));
});
