...
describe("a development URL for a client module", () => {
  ...
  it("is Vite's /@fs/ URL for a file outside it, and reads back to the file", () => {
    const url = devUrlOf("/repo/docs", "/", "/repo/packages/ui/button.js");
    expect(url).toBe("/@fs/repo/packages/ui/button.js");
    expect(fsFileOf(url)).toBe("/repo/packages/ui/button.js");
  });
