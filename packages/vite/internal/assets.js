...
import { endpointSettings, localImageEndpoint, servesRemoteImages } from "./image-endpoint.js";

/**
 * Whether `id` is `@uniflowed/web`'s `internal/image-endpoint.js`.
 *
 * By path *and* by the package it is in, because the suffix alone is a name
 * any package could have; the manifest two directories up has to say
 * `@uniflowed/web`. The answer is remembered per directory, so the manifest is
 * read once rather than once per module id.
 */
const webPackageDirectories = new Map();
function isWebImageEndpoint(id) {
  const file = id.split("?")[0];
  if (
    !file.endsWith(`${path.sep}internal${path.sep}image-endpoint.js`) &&
    !file.endsWith("/internal/image-endpoint.js")
  ) {
    return false;
  }
  const directory = path.dirname(path.dirname(file));
  if (!webPackageDirectories.has(directory)) {
    let name = null;
    try {
    ...
    webPackageDirectories.set(directory, name === "@uniflowed/web");
  }
  return webPackageDirectories.get(directory);
}

/**
 * The source `@uniflowed/web/internal/image-endpoint.js` is answered with.
 *
 * The same shape as the file, with the project's values: the endpoint's path
 * under Vite's `base` when `remotePatterns` lists anything, and `null`
 * otherwise, which is the file's own answer — so a project with no remote
 * images renders exactly what it rendered before the endpoint existed. The
 * widths and quality are the ones the endpoint will accept, defaulted the way
 * `@uniflowed/server/image` defaults them, so the `srcset` `Image` writes
 * names no width the endpoint refuses.
 *
 * @param {object} images `app.builtins.images`
 * @param {string} base Vite's resolved `base`
 */
export async function imageEndpointSource(images, base) {
  const defaults = await import("@uniflowed/server/image");
  const { widths, quality } = endpointSettings(images, defaults);
  const endpoint = servesRemoteImages(images) ? `${base}__uf/image` : null;
  return (
    "// Generated by uf:asset from app.builtins.images. See @uniflowed/web's\n" +
    "// internal/image-endpoint.js for the file this stands in for.\n" +
    `export const IMAGE_ENDPOINT = ${JSON.stringify({ path: endpoint, widths, quality })};\n`
  );
}
...
export function assetPlugin({ images = {}, fonts = {}, icons = {}, og = {}, command } = {}) {
  ...
  return {
    ...
    async load(id) {
      // `@uniflowed/web`'s idea of the image endpoint, answered with the
      // project's. See `imageEndpointSource`.
      if (isWebImageEndpoint(id)) {
        return imageEndpointSource(imagesOn ? images : { enabled: false }, base);
      }
    ...
    configureServer(devServer) {
      ...
      // `/__uf/image`, so an `Image` with a remote `src` renders under
      // `uf dev` exactly as it will under `uf start`: the same endpoint, the
      // same allow-list, the same encoder. Mounted only for a project that
      // listed remote hosts, and built on the first request for it rather
      // than with the server. See `./image-endpoint.js`.
      if (imagesOn && servesRemoteImages(images)) {
        const endpointPath = `${base}__uf/image`;
        let ready = null;
        devServer.middlewares.use((request, response, next) => {
          const url = request.url ?? "";
          const queryAt = url.indexOf("?");
          if ((queryAt === -1 ? url : url.slice(0, queryAt)) !== endpointPath) return next();
          ready ??= Promise.all([
            localImageEndpoint({ images, root, command }),
            import("@uniflowed/server/node"),
          ]);
          ready
            .then(async ([endpoint, { send }]) => {
              // The endpoint reads the query and `Accept` and nothing else, so
              // that is all the request it is handed carries.
              ...
              const answered = await endpoint(asked);
              if (answered == null) return next();
              await send(response, answered);
              return undefined;
            })
            .catch(next);
          return undefined;
