...
describe("a test file's own copy of the project's modules", () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(globalThis, FILE_SCOPE, {
      value: { shared: ["/p/node_modules/@uniflowed/test", "/p/packages/test"] },
      configurable: true,
    });
  });
  ...
  it("shares installed packages and the runner itself", () => {
    ...
    expect(fileScoped(TEST, "file:///p/packages/test/index.js", true)).toBe(
      "file:///p/packages/test/index.js",
    );
    // A directory whose name only starts like the runner's is not the runner.
    expect(fileScoped(TEST, "file:///p/packages/testing/index.js", true)).toBe(
      "file:///p/packages/testing/index.js?uf-file=7",
    );
  });

  it("leaves alone what a shared module or the worker imports", () => {
    expect(fileScoped("file:///p/packages/test/worker.js", "file:///p/app/x.js", true)).toBe(
      "file:///p/app/x.js",
    );
