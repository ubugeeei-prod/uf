// @flow
//
// What the transform cache promises, as one function per promise.
//
// The two loaders were a `for` loop around every block in one file, and that
// made it the longest file in the suite by a factor of two: 6.25 s of a 6.3 s
// run, because a file runs in one worker from end to end and this one starts
// about forty Node processes. One file per loader runs them side by side.
//
// The cases are functions here, and the `it` that names each one stays in the
// file that runs it. That is not a style choice: `uf test` decides whether a
// file holds tests by reading that file's own source, so a file whose only
// statement is a call is a file the runner walks past — 18 cases that silently
...
const REGISTER: string = path.resolve(HOST, "register.js");

/** The loader-thread hooks, which `register.js` passes over on this Node. */
const NODE_HOOKS: string = pathToFileURL(path.resolve(HOST, "internal", "node-hooks.js")).href;
...
export const IN_THREAD_HOOKS: Loader = { name: "the in-thread hooks", imports: () => REGISTER };

/** The loader thread `register()` starts, which is what an older Node gets. */
export const LOADER_THREAD: Loader = {
...
export function servesASecondRunFromDiskRatherThanCompilingAgain(loader: Loader): void {
  inAProject((root) => {
...
export function compilesAgainWhenUfItselfWasRebuilt(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
...
export function stillHasABuildsEntriesWhenThatBuildComesBack(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
...
export function doesNotServeWhatItCachedWhenItCannotTellWhichUfCompiledIt(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
    expect(run(root, { UF_BINARY: binary }, loader).stdout).toBe("first-build");
...
export function doesNotServeABuildsEntriesToTheBuildThatReplacedItMidrun(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
...
export function doesNotServeWhatItCachedThroughABinaryThatCanNoLongerRun(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
    expect(run(root, { UF_BINARY: binary }, loader).stdout).toBe("first-build");
...
export function identifiesTheBinaryAHostStartedByHandFindsOnPath(loader: Loader): void {
  inAProject((root) => {
    buildUf(root, "first-build", FIRST);
...
export function failsTheImportOfAModuleTheCompilerRefusedWithWhatTheCompilerSaid(
  loader: Loader,
): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
...
export function leavesACommonjsModuleThatSomethingRequiresToNode(loader: Loader): void {
  inAProject((root) => {
    const binary = buildUf(root, "first-build", FIRST);
