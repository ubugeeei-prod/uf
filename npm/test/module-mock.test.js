  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        const report = await ufCheck([`npm/test/${name}`, "npm/test"]);
        const types = report.typeCheck;
        expect(types.status).toBe("checked");

        const named = (file: string) =>
          types.diagnostics.filter(
            (diagnostic: $FlowFixMe) => diagnostic.primary.path === `npm/test/${name}/${file}`,
          );
...
function loaderArguments(): Array<string> {
  ...
  if (host.startsWith("node")) {
    const register = path.join(repository, "npm", "host", "register.js");
    return ["--enable-source-maps", "--import", pathToFileURL(register).href];
  }
  if (host.startsWith("bun")) {
    return ["--preload", path.join(repository, "npm", "host", "bun-preload.js")];
  }
...
describe("a mock does not outlive its file", () => {
  it(
    ...
    async () => {
      ...
      try {
        const entry = pathToFileURL(path.join(repository, "npm", "test", "index.js")).href;
        fs.writeFileSync(
