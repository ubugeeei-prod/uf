...
describe("the width tables match the CLI's", () => {
  ...
  it("is the same data on both sides, so a terminal agrees with itself", () => {
    ...
    const js = fs.readFileSync(path.join(REPO, "npm/tui/widths.js"), "utf8");
...
describe("the capability precedence matches the CLI's", () => {
  // `npm/tui/capability.js` says its precedence list is
  ...
  const js = (): string => fs.readFileSync(path.join(REPO, "npm/tui/capability.js"), "utf8");
