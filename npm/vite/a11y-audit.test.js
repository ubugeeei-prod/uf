...
// that has all three. `npm/vite/dev-channel.test.js`'s neighbours are
...
// package's own exports, the way `npm/router/streaming.test.js` does.
...
describe("the wiring into a development document", () => {
  it("adds the audit to the document DevTools already owns, and adds it last", () => {
    // The half `npm/vite/devtools.test.js` deliberately does not check.
