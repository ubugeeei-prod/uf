...
// npm/ui` — read three ways, so the file runs it once and each block
...
describe("the props a part spreads onto its element", () => {
  ...
  // — `npm/ui/internal/merge-props.js` — names `key` out of its indexer,
  ...
  // what #206 was.
  //
  // Scoped to that one family on purpose. `npm/ui` still reports
  ...
  //
  // And scoped to what the package *ships*. `uf check npm/ui` now reads
  // this file too, because this file is in `npm/ui` — that is what
  ...
  it("does not make React's key mixed", () => {
    ...
    if (run.stdout === "") {
      throw new Error(
        `\`uf check npm/ui --json\` in ${repository} printed nothing: ` +
    ...
    const tests = fs
      .readdirSync(path.join(repository, "npm", "ui"))
...
describe("a wrong child is a type error and not a review comment", () => {
  // The strongest claim `npm/ui/index.js` makes, and the one nothing here
