...
describe("the five that are one element, audited rather than asserted", () => {
  ...
  // whose whole content is `aria-*` — `npm/test/axe.test.js` proves the
...
describe("the escape hatch: which part hands its element to the caller", () => {
  ...
  // more than the contents: a part added to `npm/ui/index.js` is in none
  ...
  /** The parts `npm/ui/index.js` names, as `Dialog.Root`, and the component declared behind each. */
  ...
  function exportsOf(file: string): Map<string, string> {
    const directory = path.join(repository, "npm", "ui");
  ...
  function readPartsOfTheBarrel(): Map<string, string> {
    const source = fs.readFileSync(path.join(repository, "npm", "ui", "index.js"), "utf8");
  ...
  function sourceOf(part: string): string {
    ...
    if (component == null) {
      throw new Error(`${part} is in the table and is not exported by npm/ui/index.js`);
    }
