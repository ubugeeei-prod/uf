...
class FakeAnimation {
  ...
  /** A 150ms transition unless told otherwise. */
  constructor(options?: { endTime?: number, playState?: string }) {
    const endTime = options?.endTime ?? 150;
    this.playState = options?.playState ?? "running";
...
describe("usePresence", () => {
  ...
  it("removes a closing part at once when nothing is animating it", () => {
    // No `getAnimations`: a test DOM, a browser with no stylesheet, or `0s`
    // under reduced motion. The part goes in the commit that closed it, as it
    // did before this hook: nothing has to be awaited.
    const { rerender } = render(<Part open />);
    rerender(<Part open={false} />);
    expect(part()).toBe(null);
  });

  it("removes a closing part at once when its animation list is empty", () => {
    const { rerender } = render(<Part open />);
    animate([]);
    rerender(<Part open={false} />);
    expect(part()).toBe(null);
  ...
  it("does not wait for an animation that repeats for ever", () => {
    // A spinner inside a dialog. Waiting on it would keep the closed dialog
    // on the page for as long as the page is open.
    const spinner = new FakeAnimation({ endTime: Infinity });
    const { rerender } = render(<Part open />);
    animate([spinner]);
    rerender(<Part open={false} />);
    expect(part()).toBe(null);
  ...
  it("does not wait for a paused animation", () => {
    const paused = new FakeAnimation({ playState: "paused" });
    const { rerender } = render(<Part open />);
    animate([paused]);
    rerender(<Part open={false} />);
    expect(part()).toBe(null);
