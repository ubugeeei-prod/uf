...
| A routing call from an action (`redirect()`, `notFound()`, `unauthorized()`, `forbidden()`) answered as a failure, or answered with its exception | The JSON door answers each one as itself. A redirect is a `204` with `Location` and a `uf-action-outcome` header. It is not a `3xx`, which `fetch` would follow on its own. The other three are `404`, `401` and `403` with the same header. The body names the kind and nothing else, and none of them is reported as an action failure. The form door answers the same statuses with a fixed line of text. Every other throw is still the fixed `500` | `packages/router/action-outcomes.test.js` |
