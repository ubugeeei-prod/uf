...
| A silent all-access grant — Deno's `-A`, which uf passed unconditionally and which no later flag takes back | uf does not pass it, on any path. A Deno run starts from the toolchain's own access — the project root, the directory its packages resolve from, `.uf`, and the variables uf set on the worker — whether or not the project declared a set, because Deno's default is deny and there is no "no permission model" to hand it. Removing `-A` when a set arrived was the earlier fix and was not enough: it left every undeclared run wide open while reading as parity | `uf_test::host::tests`, `crates/uf_cli/tests/deno_host.rs` |
