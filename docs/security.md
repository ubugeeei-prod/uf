...
| A different `uf` on PATH transforming the project's modules | The driver is told which binary started it (`UF_BINARY`) and every transform goes through that one — including the ones answered from `.uf/cache/transform`, whose entries are keyed by the size and modification time of the binary that wrote them, so a different or rebuilt `uf` misses rather than inherits, and a host that cannot identify its binary caches nothing at all. `uf test` resolves the binary before it starts a worker — the running executable first, so an inherited `UF_BINARY` cannot redirect a run that knows its own path — and refuses to run at all when no route names one, rather than letting a worker fall back to a bare `uf` on `PATH` | `uf_cli::commands::vite`, `uf_cli::commands::test`, `npm/host/transform.js`, `npm/host/internal/node-hooks.js` |
| A rebuilt `uf` reporting the type errors the previous one found | `.uf/cache/check` entries are keyed by the size and modification time of the binary that wrote them, alongside every limit that can change what a check reports, a digest of the project's library definitions, and a digest of every signature the file depends on; a rebuilt `uf`, a changed limit, an edited libdef, or a moved declaration in a dependency all miss rather than inherit, and a process that cannot identify its own binary caches nothing at all. One record carries an answer per batch the file has been checked in — at most four, so a project checked many ways stays bounded — and each is believed only under the exact dependency digest it was computed with, so a whole-project run and a path-scoped one share a record without ever sharing an answer. A record that is truncated, of the wrong shape, about another file, or over its bound on answers or diagnostics is a miss | `uf_check::cache`, `uf_check::tests::cache` |
| A rebuilt `uf` reporting the lint findings the previous one found | `.uf/cache/lint` entries are keyed by the size and modification time of the binary that wrote them, the module's path and text, and everything that changes what the React Compiler is asked — which of the React tree rules were wanted, the effect-dependency validation, the compiler mode; a rebuilt `uf`, an edit, a moved file or a changed question all miss rather than inherit, and a process that cannot identify its own binary caches nothing. A record holds raw findings and each rule's level is applied when it is read, as it is to a fresh answer. A record that is truncated, over 1 MiB, of the wrong shape or version, or about another file is a miss | `uf_lint::cache`, `uf_lint::tests::cache` |
| A cache directory growing until the disk is full, and the sweep that stops it | The four answer caches — `.uf/cache/check`, `.uf/cache/lint`, `.uf/cache/transform` and `.uf/cache/task` — are each bounded at 128 MiB and swept to 96 MiB, coldest entry first. The sweep only ever unlinks — it never truncates or rewrites, so a concurrent reader sees a whole entry or none — it never removes a file used in the last minute, which is what keeps it away from a temporary another process is about to rename into place, and it does not follow symbolic links, so a link planted in a cache directory cannot make `uf` delete what it points at | `uf_infra::cache` |
...
| The build's own notes served to whoever guesses the filename | `uf-build-manifest.json`, `uf-rsc-manifest.json`, `uf-bundle-report.json` and `openapi.json` are written to `.uf/build/meta/` rather than into the output directory. Between them they name every route including the ones that were never prerendered, the source file behind each, the size of every chunk, and the route-handler schema contract; nothing reads them to answer a request. This is not a denylist in uf's two servers — the file is not there, so a static host has nothing to serve either | `uf_cli::commands::build`, `uf_cli::tests::vite` |

## Framework, RSC, and server actions

| Past failure | Structural decision in `uf` | Test |
| --- | --- | --- |
| [CVE-2025-29927](https://nvd.nist.gov/vuln/detail/CVE-2025-29927) — spoofing `x-middleware-subrequest` skips middleware, bypassing auth | No inbound request header participates in middleware dispatch: which middleware runs is decided by the request path against the directory each one guards, and by nothing else. Recursion control is internal state, never a header a client can send | `npm/router/middleware.test.js` |
...
| A routing call from an action (`redirect()`, `notFound()`, `unauthorized()`, `forbidden()`) answered as a failure, or answered with its exception | The JSON door answers each one as itself. A redirect is a `204` with `Location` and a `uf-action-outcome` header. It is not a `3xx`, which `fetch` would follow on its own. The other three are `404`, `401` and `403` with the same header. The body names the kind and nothing else, and none of them is reported as an action failure. The form door answers a redirect with a bodyless `303` and `Location`, as before, and answers the other three with their statuses and a fixed line of text. Every other throw is still the fixed `500`. In the browser the reference follows only an `http:` or `https:` `Location`, resolved against the page the call was made from | `npm/router/action-outcomes.test.js` |
...
| A report whose size a cloned repository decides: one added directive moves every module above it into the client bundle at once, and rule 1 makes a cloned repository's source untrusted | `uf dev` prints the count in full and at most twelve of the chains behind it, so the report is bounded by uf rather than by the graph. Each chain is a simple path — one predecessor per module in a breadth-first walk — so it cannot repeat a module or outrun the module count, and the walk itself terminates on a cyclic import graph because that is the only kind this crate has | `uf_cli::commands::dev::rsc::tests`, `uf_rsc::graph` |
...
| A page rendered as something other than what the request's path names — the page underneath an intercepting route's slot — without the middleware that guards it having been asked | An intercepted navigation's payload is two pages: the one its URL names, in the slot, and the one `uf-intercepted-from` names, underneath, with that page's loader run. The header is the client's to write, so the middleware runner asks every middleware guarding the page underneath as well — as a `GET` for that page, root first, including one that already ran for the URL, because a middleware that reads the pathname has only been asked about the other path. If any of them answers or rewrites, the header is taken off and the page the URL names renders on its own; a header that is not a path on this origin is taken off too. Every front door renders from the request the runner hands back, never from the header as it arrived. Checking authorization in the loader as well (`unauthorized()`, `forbidden()`) is still the stronger design: a loader protects its data whatever renders it | `npm/router/middleware-rewrite.test.js` |
| DOM XSS through a navigation: `redirect(searchParams.get("next"))` or `router.push(value)` handed to `location.assign`/`location.replace`, which run a `javascript:` URL in the page | `RedirectError` refuses a `javascript:`, `vbscript:` or `data:` URL when it is built, so the server's `Location`, a single-page application's `location.replace` and a form's `303` share one check; `router.push`/`replace` refuse the same before any path that could end in `location`, and `Link`'s fall-back to the browser skips a URL the router refused. The scheme is read with the WHATWG URL parser, so the spellings a browser forgives — leading spaces and control characters, tabs inside the word, any case — are the same scheme here. React 19 refuses `javascript:` in `href`; this is the half of the router React never sees. An application that redirects to a query parameter should still allow only its own paths: a refused scheme is not an allow-list | `npm/router/routing.test.js`, `npm/router/navigation.test.js` |
| An open redirect on the application's own domain: a path that opens with two slashes — the link `https://app.example//evil.example/x/`, or `/\evil.example`, which a URL parser reads the same way — tidied by a trailing-slash rule or a post/redirect/get into `Location: //evil.example/x`, a network-path reference the browser follows to another host | Every `Location` uf writes from the request's own path has its leading run of `/` counted down to one first: `app.router.trailingSlash`'s `308` and the `303` a form posted before hydration is answered with. A counted loop, not a pattern | `npm/server/routing.test.js`, `npm/router/form-action.test.js` |
| A request-target that names a host: `new URL("//evil.example/x", base)` is `http://evil.example/x`, so a Node door that resolved the target against `Host` handed every check reading `new URL(request.url).host` — the OAuth routes' same-origin test, an application's `new URL("/sign-in", request.url)` redirect — somebody else's origin | The Node doors (`@uniflowed/server/node`, and `@uniflowed/vite`'s for `uf dev` and `uf preview`) join `Host` and the target's path as text, so the authority is `Host`'s alone. An absolute-form target keeps its path and loses its authority, and a `Host` holding a path, a user name, a query or a fragment is not believed | `npm/server/serve.test.js` |
| SSRF via WebSocket upgrade ([CVE-2026-44578](https://nvd.nist.gov/vuln/detail/CVE-2026-44578)) | uf has no proxying upgrade, and the one it does have cannot become one: `upgradeWebSocket(request)` upgrades the *inbound* connection the host is already answering, takes no address, and reaches no upstream. The host's own upgrader is a value the deployment passes where the server is built, never a name resolved from a request. A proxying upgrade would need the allowlist in its first commit rather than after one | `npm/server/transports.test.js` |
| Image optimizer: unbounded disk cache ([CVE-2026-27980](https://nvd.nist.gov/vuln/detail/CVE-2026-27980)), CPU exhaustion from remote images ([CVE-2024-47831](https://nvd.nist.gov/vuln/detail/CVE-2024-47831), [CVE-2026-64644](https://nvd.nist.gov/vuln/detail/CVE-2026-64644)), cache deception | There is no endpoint until `app.builtins.images.remotePatterns` lists a host. It answers only the widths and qualities the project declared, runs at most four encodes at once with a bounded queue behind them (`503` past it), and bounds decode work by `MAX_VARIANT_PIXELS` read from the header. A decode is not interrupted once it has started: what bounds a pathological file is the pixel budget and the four-at-once gate, not a timer. Variants live in a `CacheStore` — `MAX_MEMORY_VARIANTS` entries in memory, or the durable provider the project already bounds — keyed on the parsed source URL, the width, the quality and the *negotiated* format, never the raw `Accept`, and answered with `Vary: Accept`. The rest is the "Image and font transformation" section below | `npm/server/image.test.js`, `crates/uf_cli/tests/vite.rs` |
| A draft-mode cookie anybody can set for themselves — a flag rather than a token, so unpublished content is gated by a value you can type | The value is an expiry and an HMAC-SHA256 over it — domain-separated `uf-draft-v1`, length-prefixed, keyed with the deployment's secret, the same construction `crates/uf_rsc/src/action.rs` argues for under a different name — compared in constant time, with the expiry *inside* the signature so a holder cannot extend it and checked against uf's clock as well as by `Max-Age`. `__Host-` prefixed, `Secure`, `HttpOnly`, `SameSite=Lax`, `Path=/`, one name for every scheme so there is no second name a subdomain could plant. The key is `UF_DRAFT_SECRET`, refused below 32 bytes rather than stretched, and a per-process one otherwise with a `warn` saying what that costs | `npm/router/route-handler.test.js` |
| A cookie or a response header set from inside a render, at a moment when the headers may already be on the wire | `headers()` and `cookies()` are read-only, and `draftMode().enable()` — the one case that needs a response — is allowed only where a response is being produced: a route handler or a server action, marked by `asResponder`. Anywhere else, a guard included, it is a named `DraftModeError` rather than a decision nothing writes down | `npm/server/server.test.js`, `tests/library/request-lifecycle.test.js` |
| Draft content served out of a shared cache, or a published document served to somebody who came to see the draft | A request in draft mode never reaches the route cache and is never answered with a prerendered document. **Why** is written once, in `npm/server/internal/draft.js`'s `prerenderedMayAnswer`, and every front door asks it rather than deciding again: `uf start`'s static handler, the compiled binary's embedded index, a worker's assets binding, and `uf preview` — where Vite's own file middleware runs in front of anything uf mounts behind it, so uf mounts the question in front of Vite instead. Documents only; a chunk is the same bytes either way. The other direction holds too: reading `draftMode()` counts as reading request state, so a render that consulted it is never stored | `npm/server/cache.test.js`, `npm/server/serve.test.js`, `npm/server/standalone.test.js` |
| A Flight payload opened as a document: `<route>/__uf.flight` holds the page's strings with their markup, at a URL anyone can link to | Every door answers a payload as `text/x-component` with `X-Content-Type-Options: nosniff`: a rendered one (`flight()` in `@uniflowed/router/rsc/ssr`, which `uf dev`, `uf start` and every adapter send), a prerendered file (`uf start`'s static half, and `uf preview`). No current browser sniffs that type into HTML, and `nosniff` makes that a promise rather than a property of today's browsers | `npm/server/serve.test.js`, `crates/uf_cli/tests/vite.rs` |
| A loader's thrown value in the page's payload: React's production rule — the digest crosses, the message and stack do not — covers an `Error`, and a thrown string or plain object (an API client's `{ message, details, hint }`) is serialised as the data it is | The route's error is made crossable before the payload is rendered: a thrown value that is not an `Error` is wrapped in one, so the same rule applies to it. The host is still told the original value | `npm/router/flight-error.test.js` |
| XSS via CSP nonce handling and `beforeInteractive` scripts | A nonce is minted per response from 128 bits of `crypto.getRandomValues`, and never taken from a request header — an inbound nonce is a value the client chose, and a client that picks the nonce picks the one string that would make a strict policy admit the script it injected. It is never reused: a render that carried one is refused by the route cache, so no two responses can share it. Every `<script>` uf writes for that response carries it — the client entry, the Flight payload chunk elements, and, through React's own `nonce` option, every inline script React emits to reveal a `<Suspense>` boundary. A project reads the same value with `nonce()`, or names `{uf.nonce}` in an `app.router.headers` rule, so its policy and uf's markup cannot disagree. A prerendered document has no request and therefore carries no nonce, by construction rather than by omission: a per-request value written into a file is wrong for every request after the first, so a prerendered route under a nonce policy needs hashes — `infra/cloudflare/workers/docs.js` is the worked example. Observed in a browser, not only argued: under `script-src 'nonce-…' 'strict-dynamic'` a server-rendered route loaded and hydrated with every uf script nonced, while a parser-inserted inline script without the nonce was refused | `npm/router/csp-nonce.test.js`, `tests/library/deploy.test.js` |
| The `application/json` and `application/ld+json` blocks uf renders through React carry no nonce, so a strict policy might refuse them and take hydration with them | It does not, and this is settled by observation rather than by reading a specification. HTML classifies a `<script>` whose type is neither a JavaScript MIME type nor `module` as a *data block* and never executes it, so `script-src` does not gate it. Checked in a real browser against a real deployment: a page served under `script-src 'nonce-…' 'strict-dynamic'` carrying three parser-inserted un-nonced scripts — one executable, one `application/json`, one `application/ld+json` — reported exactly one violation, the executable one, and both data blocks stayed in the DOM with their text readable. The reason they are left un-nonced is hydration rather than CSP: React compares a `nonce` during hydration through the DOM node's `nonce` IDL property, so a nonce the server wrote and the client did not repeat is a mismatch, and the client cannot learn the value without a carrier that would hand it to an HTML injection too — which is the mitigation browsers blank the attribute for | `npm/router/csp-nonce.test.js` |
...
`npm/router/internal/action-wire.js` is the only place that applies it:
...
| An unbounded response from a compromised or hostile token endpoint | The body is read in chunks against a 64 KiB ceiling and the stream is cancelled above it, rather than being buffered whole by `response.json()` | `npm/server/internal/oauth.js` |
| An `expires_in` that is not a lifetime — `NaN`, an infinity, a negative number, or one far enough out to leave the range an instant can hold | The value is bounded before it is added to the clock, and anything outside it reads as "the provider did not say" rather than as a `RangeError` thrown out of the middle of a sign-in. A token endpoint is a third party and its answer is untrusted input, exactly as a browser's request is | `npm/server/oauth.js`, `npm/server/oauth.test.js` |
...
| Concern | Decision in `uf` | Where |
| --- | --- | --- |
| A credential in a log line. A token in the log store is read by more people, kept for longer, and replicated further than the store it came from | A closed table of field names whose value is never printed, matched after normalising case, `-`, `_` and `.`, and applied at every depth. A table of *names* rather than a test of values: recognising "this looks like a JWT" is a regex over untrusted input, which rule 5 forbids, and a heuristic that misses once has published the token | `npm/server/internal/log.js`, `npm/server/log.test.js` |
| Log injection — a newline in a path or a user agent closing its own record and opening a fabricated one | Every string in a record loses its control characters and is cut to a fixed length before it is formatted, the same treatment `uf_pm::progress` gives registry text before drawing it | `npm/server/internal/log.js`, `npm/server/log.test.js` |
| A query string in an access line: a return path, a search term, a signed URL, an OAuth `code` and `state` | The access line carries `url.pathname` and never the search. A host writing that line does not know which route it is logging, so the only rule it can apply is the one that is right for every route | `npm/server/log.js`, `npm/server/log.test.js` |
| A field named `level` or `msg` relabelling a record's own severity | The record's own keys are written after the fields, so a field of the same name cannot displace one | `npm/server/internal/log.js`, `npm/server/log.test.js` |
| An inbound `X-Request-Id` becoming the id `uf` correlates by | `uf` generates the request id and never takes it from the request. A client-chosen id can be identical on a million requests, which defeats the only thing an id is for, and it lands in a log line | `npm/server/internal/context.js` |
| A request id rendered into a document that is then cached, so every later visitor is told they are the first one | `requestId()` counts as a read of request state, exactly as `cookies()` does, so the route cache refuses to store the render. `logger()` deliberately does not: an id that reached a log line has not made the document personal | `npm/server/index.js`, `npm/server/log.test.js` |
| A log line written into `uf`'s own control channel | The default sink writes every level to `console.error`. In the process that runs `uf start` and `uf preview`, stdout is `@uniflowed/vite`'s JSON event channel and `console.info` goes to stdout on Node, so choosing the stream by level would put a log line in the middle of a protocol — intermittently, and only under traffic | `npm/server/internal/log.js` |
| A request Node's own parser refuses, answered `400` by the runtime and recorded nowhere | Every server uf ships — `serve`, and the compiled binary's — attaches a `clientError` handler that writes the same `400` the default one does and reports it at `warn` with the error's `code` and nothing else; Node puts the offending bytes on `error.rawPacket`, and those are whatever the client sent | `npm/server/log.test.js` |
| That same line as an amplifier: a malformed request is two dozen bytes to send, and one line to write is a disk and a retention window somebody else gets to spend | Twenty lines a minute per server, then a count of what the budget hid written once when the next window opens. A flood therefore costs a fixed number of lines and still says how big it was. The `400` is answered whether or not a line was written, because the budget bounds the log and not the protocol | `npm/server/log.test.js` |
| Unbounded work from a field a handler passed to a logger | Values are walked to a fixed depth with a fixed number of keys per object and entries per array, and strings are cut | `npm/server/internal/log.js`, `npm/server/log.test.js` |
...
| Terminal escape sequences in **anything a diagnostic prints out of a checkout** — the file name, the message quoting a module or an import specifier, the source line under the caret, the label after it | A repository is attacker-authored input to a terminal by the same argument an image from a dependency is to a decoder, and a diagnostic prints four different pieces of it. `uf_term::text` prepares two shapes: `safe_path` cuts from the head, because a path is identified by its tail, and `safe_message` cuts from the tail, because a sentence is read from its start. Both drop every `Cc` scalar, so `\x1b`, `\r` and the one-byte C1 `CSI` cannot survive. `uf_term::diagnostic` applies them to every part of a code frame, which is the one place `uf lint`, `uf check`, `uf build`, `uf test` and the dev server's diagnostics all print through; `dev/rsc.rs` prepares the one message that bypasses the frame. The source line is filtered rather than rewritten, and the caret cannot move: every control character but the tab is already zero columns wide | `uf_term::text::tests::a_path_cannot_move_the_cursor`, `a_message_cannot_move_the_cursor`, `a_message_wider_than_the_cap_is_cut_from_its_tail`, `uf_term::diagnostic::tests::a_source_line_is_sanitised_without_moving_the_caret`, `a_frame_message_cannot_be_made_to_clear_the_screen`, `a_caret_label_cannot_move_the_cursor` |
...
| A silent all-access grant — Deno's `-A`, which uf passed unconditionally and which no later flag takes back | uf does not pass it, on any path. A Deno run starts from the toolchain's own access — the project root, the directory its packages resolve from, `.uf`, the one `uf` binary its loader compiles through, and the variables uf set on the worker — whether or not the project declared a set, because Deno's default is deny and there is no "no permission model" to hand it. Removing `-A` when a set arrived was the earlier fix and was not enough: it left every undeclared run wide open while reading as parity | `uf_test::host::tests`, `crates/uf_cli/tests/deno_host.rs` |
...
| The grants uf makes for itself being invisible, so nobody can check them | `uf explain test` prints, per permission, how many entries the project declared, how many uf added, and which flag enforces it. On Node the two additions are `--allow-worker` (the Flow loader compiles on a thread) and `--allow-child-process` (every module is transformed by a `uf transform` child), neither of which Node can scope — so a test can still start a program, and `docs/hosts.md` says so in as many words rather than leaving it to be discovered | `crates/uf_cli/tests/permissions.rs` |
| An agent's `paths` reaching outside the project over `uf mcp` — `uf_fmt_write` with `../outside` rewriting the directory beside it | Every tool refuses, by name and before anything is read, a `paths` entry that is absolute, whose `..` climbs above the project root, or whose nearest existing part resolves outside the resolved root through a symbolic link, compared by component. The command line is deliberately not held to it: `uf fmt ../elsewhere` is what the person running it typed, and an agent's argument was written by somebody the server cannot ask | `uf_cli::commands::mcp::tests`, `tests/library/mcp.test.js` |
...
| A `.env` served over HTTP by the dev server | Vite's built-in `server.fs.deny` covers `.env` and `.env.*` and no project configuration removes a built-in entry | `npm/vite/driver.js` |
...
| Risk | Structural decision in `uf` | Test |
| --- | --- | --- |
| A memory-safety bug in an image codec | Every decoder and encoder is pure Rust with no `unsafe` of uf's own and no C library linked: `png`, `zune-jpeg`, `image-webp` and `ravif`/`rav1e` by way of `image`, chosen with `default-features = false` so no codec uf does not emit is compiled in at all | `uf_assets::image`, `uf_assets::variant` |
...
### The request-time image endpoint

`/__uf/image` fetches a URL a query string names, from inside the network the
server runs in, and decodes what comes back. That is server-side request
forgery and a decoder fed by a stranger in one feature, so it arrived with its
allow-list in the same commit, and it exists only for a project that lists a
host. `@uniflowed/server/image` is the endpoint on every door — `uf dev`,
`uf preview`, `uf start` and each `--adapter` — so each row holds on all of
them unless it says otherwise.

| Risk | Structural decision in `uf` | Test |
| --- | --- | --- |
| SSRF to a host nobody listed | The allow-list is asked before anything else, and nothing else admits a host: exact names, a leading `*.` for one label or `**.` for any, HTTPS unless `http` is written, the default port unless a port is written, a path matched segment by segment against the *parsed* URL. A URL with credentials in it is refused. A bare `*`, a wildcard inside a name or a segment, and a capitalised host are refused when the config is read, in Rust and again in JavaScript, and no regular expression is built from any of it | `npm/server/image.test.js`, `uf_config::tests` |
| A redirect out of the allow-list | The platform never follows a redirect. Each `Location` is resolved against the URL that sent it and is asked of the allow-list on its own, three hops at most; the one that leaves it is a `403` and is never requested | `npm/server/image.test.js`, `crates/uf_cli/tests/vite.rs` |
| A listed name that resolves to a private address, including DNS rebinding | The name is resolved with every address it has, and one loopback, private, link-local, CGNAT, documentation, multicast or reserved answer refuses the request — IPv4-mapped, NAT64 and 6to4 forms judged by the IPv4 address inside. The connection is then made to the address that was checked, with the name as the TLS server name and the `Host`, so a second lookup cannot answer differently. A Worker has no resolver and fetches from Cloudflare's network; there the allow-list and redirect rows hold and this one does not exist. `dangerouslyAllowPrivateAddresses` turns it off, for a test serving its own images | `npm/server/image.test.js` |
| An enormous source, or one that never ends — [CVE-2025-59471](https://nvd.nist.gov/vuln/detail/CVE-2025-59471), and [CVE-2026-44577](https://nvd.nist.gov/vuln/detail/CVE-2026-44577) for a local one | `MAX_SOURCE_BYTES` (32 MiB) counted as the body arrives, not trusted from `Content-Length`, and the read cancelled the moment it is passed; fifteen seconds for the whole fetch. A local image is not fetched at all: a relative `url` is a `400`, because an image in the project is the build's work | `npm/server/image.test.js` |
| A decompression bomb at request time | `uf_assets::variant` reads the dimensions from the header and refuses past `MAX_VARIANT_PIXELS` (64 megapixels) before a buffer exists, and decodes under `image::Limits` as well, so a header that lies fails in the decoder rather than in the allocator | `uf_assets::variant` |
| An SVG, or HTML labelled as an image, served from the application's origin | Only PNG, JPEG and WebP, decided by the first bytes rather than by the origin's `Content-Type`; everything else is a `415`. What is served back is a re-encode, with `X-Content-Type-Options: nosniff` and `Content-Security-Policy: default-src 'none'; sandbox` | `npm/server/image.test.js`, `uf_assets::variant` |
| A memory-safety bug in the request-time encoder | The AVIF encoder is `rav1e` through `ravif`, pure Rust, with `image`'s `nasm` feature off so no assembly is linked; the other encoders are the build's. A deployed directory without `uf` encodes through Cloudflare's Images binding or a module the project names, and that module is the project's own trust decision | `uf_assets::variant` |
| A header-driven cache poisoning — one browser's AVIF served to another that cannot draw it | The format is negotiated from `Accept` into one of two values before it is a key, and the response carries `Vary: Accept` for any shared cache in front | `npm/server/image.test.js` |
| One person's image served to another from the variant cache — [CVE-2025-57752](https://nvd.nist.gov/vuln/detail/CVE-2025-57752) | Nothing of the incoming request but its query and `Accept` reaches the fetch: no cookie, no `Authorization`, no other header is forwarded, so an origin cannot answer one visitor differently from the next, and the key is every parameter the answer depends on. An origin that says `private` or `no-store` is answered and not kept | `npm/server/image.test.js` |
| A remote file delivered as a download with a name and type the origin chose — [CVE-2025-55173](https://nvd.nist.gov/vuln/detail/CVE-2025-55173) | The origin's headers never reach the answer. What is served is uf's own re-encode, with uf's `Content-Type` for it, no `Content-Disposition`, and the body of anything that is not an image never re-served at all | `npm/server/image.test.js` |
| Metadata leaking from an upload — a camera's GPS position | Every answer is a re-encode: EXIF orientation is applied, and the rest of the metadata is not written | `uf_assets::variant` |
...
| **[CVE-2018-6341](https://nvd.nist.gov/vuln/detail/CVE-2018-6341)** — React DOM server-side attribute injection | The renderer is React's own, and uf adds no attribute path around it. `Metadata` values reach `<meta content>` as React children, which React escapes | `npm/router/internal/runtime.js` |
| Path traversal in static file serving | Decoded, NUL refused, `path.resolve`, and then required to be the root or under it — the check after normalisation rather than before | `npm/server/node.js` |
| **Symlink escape** from a served directory | The file that is opened is realpath'd and required to be inside the realpath'd root, so a link that reads as inside and points outside is a 404 — indistinguishable from a file that is not there. A link that stays inside still works | `npm/server/node.js`, `npm/server/serve.test.js` |
