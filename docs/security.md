...
| A nonce policy on a route the build prerendered: the header names a fresh per-request nonce and the document on disk was written without one, so the page's own client entry is refused and nothing hydrates | **Not prevented**, and named here because it is the combination that fails silently in production rather than loudly in a build. `{uf.nonce}` in `app.router.headers` applies to every response a front door answers, prerendered documents included, and no door can tell a file from a render. Observed: a prerendered `/` answered `Loading the script '/assets/client-….js' violates … 'script-src 'nonce-…' 'strict-dynamic''` and stayed un-hydrated, while a server-rendered route on the same deployment worked. A project that prerenders must scope the rule to the routes a server renders, or serve the prerendered ones under a hash policy | todo |
