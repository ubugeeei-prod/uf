...
| A documentation site that argues about CSP and serves none — an injected `<base>`, a framed copy, an `<object>` | `infra/cloudflare/workers/docs.js` sends a policy derived from what the site actually loads rather than from a template: every fetch directive is `'self'`, because the build serves no font, no `url()` in any stylesheet, no `data:` URI, no cross-origin `fetch` and no frame; `object-src`, `frame-src` and `frame-ancestors` are `'none'`; and `base-uri` is pinned, without which one injected `<base>` redirects every relative script URL in the document and `script-src 'self'` does not stop it. `style-src` needs no `'unsafe-inline'` — the build emits no inline `style` attribute, so the commonest reason a policy is weakened does not apply. The one inline script is the theme bootstrap, which must run before first paint, and is allowed **by hash**; the test computes that hash from the exported string, so editing the bootstrap fails rather than silently breaking the theme | `tests/library/docs-csp.test.js` |
