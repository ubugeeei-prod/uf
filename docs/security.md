...
| Terminal escape sequences in a **file name from a checkout**, drawn by a reporter that prints a path | A repository is attacker-authored input to a terminal by the same argument an image from a dependency is to a decoder. `uf_term::safe_path` gives a path the treatment `uf_pm::progress` gives registry text — every `Cc` scalar dropped, so `\x1b`, `\r` and the one-byte C1 `CSI` cannot survive, then capped to 120 columns, elided from the head so the filename is what is kept. Every reporter that draws a code frame goes through `uf_term::diagnostic`, which applies it to the header: `uf lint`, `uf check`, `uf build`, and the dev server's diagnostics, including the ones `uf transform` produces. The three group headers that print a path beside a frame apply it themselves. A `\` separator and a filename in any script are untouched — a sanitiser that dropped what it did not recognise would name the wrong file | `uf_term::text::tests::a_path_cannot_move_the_cursor`, `a_path_keeps_every_character_that_is_not_a_control`, `a_path_wider_than_the_cap_is_elided_from_its_head`, `uf_term::diagnostic::tests::a_frame_header_cannot_be_made_to_clear_the_screen` |
