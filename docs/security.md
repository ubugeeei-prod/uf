...
| A routing call from an action (`redirect()`, `notFound()`, `unauthorized()`, `forbidden()`) answered as a failure, or answered with its exception | The JSON door answers each one as itself. A redirect is a `204` with `Location` and a `uf-action-outcome` header. It is not a `3xx`, which `fetch` would follow on its own. The other three are `404`, `401` and `403` with the same header. The body names the kind and nothing else, and none of them is reported as an action failure. The form door answers a redirect with a bodyless `303` and `Location`, as before, and answers the other three with their statuses and a fixed line of text. Every other throw is still the fixed `500`. In the browser the reference follows only an `http:` or `https:` `Location`, resolved against the page the call was made from | `packages/router/action-outcomes.test.js` |
...
| Concern | Decision in `uf` | Where |
| --- | --- | --- |
| A credential in a log line. A token in the log store is read by more people, kept for longer, and replicated further than the store it came from | A closed table of field names whose value is never printed, matched after normalising case, `-`, `_` and `.`, and applied at every depth. A table of *names* rather than a test of values: recognising "this looks like a JWT" is a regex over untrusted input, which rule 5 forbids, and a heuristic that misses once has published the token | `packages/server/internal/log.js`, `packages/server/log.test.js` |
| Log injection — a newline in a path or a user agent closing its own record and opening a fabricated one | Every string in a record loses its control characters and is cut to a fixed length before it is formatted, the same treatment `uf_pm::progress` gives registry text before drawing it | `packages/server/internal/log.js`, `packages/server/log.test.js` |
| A query string in an access line: a return path, a search term, a signed URL, an OAuth `code` and `state` | The access line carries `url.pathname` and never the search. A host writing that line does not know which route it is logging, so the only rule it can apply is the one that is right for every route | `packages/server/log.js`, `packages/server/log.test.js` |
| A field named `level` or `msg` relabelling a record's own severity | The record's own keys are written after the fields, so a field of the same name cannot displace one | `packages/server/internal/log.js`, `packages/server/log.test.js` |
...
| A request id rendered into a document that is then cached, so every later visitor is told they are the first one | `requestId()` counts as a read of request state, exactly as `cookies()` does, so the route cache refuses to store the render. `logger()` deliberately does not: an id that reached a log line has not made the document personal | `packages/server/index.js`, `packages/server/log.test.js` |
...
| A request Node's own parser refuses, answered `400` by the runtime and recorded nowhere | Every server uf ships — `serve`, and the compiled binary's — attaches a `clientError` handler that writes the same `400` the default one does and reports it at `warn` with the error's `code` and nothing else; Node puts the offending bytes on `error.rawPacket`, and those are whatever the client sent | `packages/server/log.test.js` |
| That same line as an amplifier: a malformed request is two dozen bytes to send, and one line to write is a disk and a retention window somebody else gets to spend | Twenty lines a minute per server, then a count of what the budget hid written once when the next window opens. A flood therefore costs a fixed number of lines and still says how big it was. The `400` is answered whether or not a line was written, because the budget bounds the log and not the protocol | `packages/server/log.test.js` |
| Unbounded work from a field a handler passed to a logger | Values are walked to a fixed depth with a fixed number of keys per object and entries per array, and strings are cut | `packages/server/internal/log.js`, `packages/server/log.test.js` |
...
| A header-driven cache poisoning — one browser's AVIF served to another that cannot draw it | The format is negotiated from `Accept` into one of two values before it is a key, and the response carries `Vary: Accept` for any shared cache in front | `packages/server/image.test.js` |
| One person's image served to another from the variant cache — [CVE-2025-57752](https://nvd.nist.gov/vuln/detail/CVE-2025-57752) | Nothing of the incoming request but its query and `Accept` reaches the fetch: no cookie, no `Authorization`, no other header is forwarded, so an origin cannot answer one visitor differently from the next, and the key is every parameter the answer depends on. An origin that says `private` or `no-store` is answered and not kept | `packages/server/image.test.js` |
| A remote file delivered as a download with a name and type the origin chose — [CVE-2025-55173](https://nvd.nist.gov/vuln/detail/CVE-2025-55173) | The origin's headers never reach the answer. What is served is uf's own re-encode, with uf's `Content-Type` for it, no `Content-Disposition`, and the body of anything that is not an image never re-served at all | `packages/server/image.test.js` |
