...
| A `.env` in a cloned repository steering the toolchain — `UF_BINARY` names the binary every module is transformed through | The project's values are applied to a child process *before* uf's own variables, so a file that names one is overwritten rather than obeyed | `uf_cli::commands::driver`, `uf_test::host` |
