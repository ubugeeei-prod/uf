...
| Claim | Today | |
| --- | --- | --- |
| Runs on every runtime | Node and Bun, each with a test that starts the binary. Deno runs a suite through an ahead-of-time transform and an import map — *experimental*, because a module uf cannot enumerate is still Flow there and a watch loop is refused. Edge starts the generated Cloudflare Worker under Wrangler local — also *experimental*, because there is still no source-level host, Flow loader or remote worker coverage. The per-host matrix is [docs/hosts.md](hosts.md) | [#246](https://github.com/ubugeeei-prod/uf/issues/246) |
