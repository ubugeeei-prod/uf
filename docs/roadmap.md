...
| Claim | Today | |
| --- | --- | --- |
| Runs on every runtime | Node, Bun and Deno, each with a test that starts the binary. Deno loads Flow through `registerHooks`, which it implemented in 2.8, and runs `uf dev`, `uf build`, `uf start` and `uf test` there; what it still lacks is coverage, and a native addon a test requires after the hook is installed. Edge starts the generated Cloudflare Worker under Wrangler local — also *experimental*, because there is still no source-level host, Flow loader or remote worker coverage. The per-host matrix is [docs/hosts.md](hosts.md) | [#246](https://github.com/ubugeeei-prod/uf/issues/246) |
| Builds a standalone binary | `uf build --compile` writes one on the project's own Capability JS Host — Bun, or Node 25.5 and newer through `--build-sea`; `--target` cross-compiles on Bun's backend. What is left is Deno, which has `deno compile` and no backend here, and that no cross-built binary has ever been *run*: the test asserts the ELF header, because the build machine cannot execute one | [#246](https://github.com/ubugeeei-prod/uf/issues/246) |
| Deploys anywhere | `uf build --adapter` writes for `node`, `bun`, `deno`, `container`, `edge`, `serverless` and `static`, and no server output has ever been deployed to a real platform | [#391](https://github.com/ubugeeei-prod/uf/issues/391) |
...
- RSC module graph split. **The module level is done**: routes render as React
  ...
  chain of imports from the client boundary that put it there. Server functions
  passed as props cross as server references (#1359). Still open: intercepting
  routes under the payload router. **The split can now be read**:
...
- Server action transform and request bridge. **Done for a module export**: a
  ...
  `uf build`, and in all six deploy adapters, which serve it out of one
