...
- [ ] Default app execution to runtime-agnostic Capability JS Hosts: Node.js,
      Deno, and Bun. All three run a uf project, each through a module hook —
      Deno's is `node:module`'s `registerHooks`, from Deno 2.8, where it used to
      be an ahead-of-time transform and an import map. `docs/hosts.md` grades
      all three **implemented** and names what each still lacks, and
      `uf_runtime::HOSTS` is the source of truth. See #246.
...
- [x] Support PPR, SSR, SSG, and ISR. All four are **Implemented** and
      are selected per route: a page is prerendered, or rendered per request by
      `uf start`, `uf preview` or a deploy adapter, and `dynamic =
      "force-dynamic"` is how a page with no parameters chooses the second. A
      prerendered page that states a lifetime is served from the build's
      document and regenerated by the server once the lifetime passes, through
      the route cache (#950). `app.rendering.modes` is the project's allowlist,
      and a route it cannot honour is a build error naming it rather than a
      document that never existed (#336, #385). A prerendered page that reads
      `cookies()`, `headers()` or `draftMode()` inside a `<Suspense>` boundary
      is written as a static shell, and `uf start`, `uf preview` and every
      adapter that streams send the shell before rendering anything and then
      stream the boundary rendered for the request (#950); `serverless` and
      `static` refuse such a page by name, and a read outside every boundary
      fails the build naming the route.
- [x] Keep route, fetch, action, and data cache defaults OFF. `route`, `fetch`
      and explicit `cacheFunction` calls under `data` reach a store with lifetime
      and tag revalidation; `actions` is refused. See #277, #961 and
