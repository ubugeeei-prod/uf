...
A borrowed argument should borrow that compact result. `.into_string()` is an
explicit compatibility boundary for an API that still requires `String`; it
still allocates for inline results and is not a performance improvement by
itself. Do not introduce it into a repeated append loop.
...
| memchr | Line boundaries and slash normalization without an intermediate `String` |
...
`index_vec`, `ecow`, `roaring`, `bstr`, and `aho-corasick` remain candidates
for typed dense graphs, cloned immutable strings, sparse integer sets, byte
