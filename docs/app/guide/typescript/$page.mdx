...
(`quickfix` and `source.fixAll.uf`), `textDocument/hover`, and
`textDocument/completion` — which answers in `uf.config.js` only, from
`@uniflowed/config`'s own type. There is no rename, no go-to-definition, no
find-references, no auto-import, and no completion in your own modules.
