...
of it. What it can do is the diagnostics it pushes — the linter's when a document
opens or changes, and Flow's type errors, the ones `uf check` reports, once
typing pauses — and what it advertises in `initialize`: `textDocument/formatting`,
...
and `textDocument/completion`. Hover, the definitions and completion outside
`uf.config.js` are answered by Flow's own inference over the whole project,
kept in memory: the type under the cursor, where a name or a type is declared —
across files and into `node_modules` — and the members of a value after `.`.
There is no rename, no find-references, no signature help, no inlay hints, no
document symbols and no auto-import. Against TypeScript 7's own server, which
advertises all of those, that is most of the gap.
...
```
  › zod: 17 holes typed as any, 224 Flow errors inside its translation
