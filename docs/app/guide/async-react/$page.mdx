...
**What you will be able to do:** place Suspense boundaries, keep the current
page visible through a transition, show optimistic updates, and choose where
each boundary goes.

**What you need first:** [Routing](/guide/routing), for `$loading.js` and
loaders, and [Server Components](/guide/server-components) for the client
boundary these hooks live behind.
...
The boundary's position is the contract. A `$loading.js` beside a segment wraps
that segment's page and everything below it; layouts above the segment stay
visible, and layouts inside the segment wait with the page. The route tree is
therefore also the streaming tree:

```text
app/$layout.js                 always in the shell
app/articles/$loading.js       fallback in the shell
app/articles/[slug]/$layout.js streamed with the page when ready
app/articles/[slug]/$page.js   streamed with the page when ready
```

A root `$loading.js` works even when the app has no root layout. That edge is
easy to get wrong because `<Suspense>` renders no element of its own, and React
cannot flush a boundary that is a direct child of the render root. uf adds the
minimal host frame for that no-layout root case, so the fallback still leaves in
the first chunk rather than waiting for the page it was supposed to replace.
...
type Message = {| readonly id: string, readonly body: string, readonly sending?: boolean |};
...
[Server actions](/guide/server-actions) are the functions a transition calls.
