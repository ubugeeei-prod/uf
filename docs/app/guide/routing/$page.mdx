...
- a slot holds pages, layouts, templates, loading fallbacks, nested slots and
  one default — `$error.js`, `$not-found.js`, `$route.js` or `$middleware.js`
  inside one is refused, because uf's slots do not carry those boundaries or
  request handlers of their own yet
