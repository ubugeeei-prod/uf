...
A directory named `(.)photo`, `(..)photo`, `(...)photo`, `(..)(..)photo` or
`(..)(..)(..)photo` is an **intercepting route**. uf does not have interception
— a navigation carries where it is going and not where it came from — so the
directory is refused by name, by `uf build`, by `uf dev` and by `uf lint`.
...
segment": `app/@team/$page.js` served `/@team`,
`app/feed/(.)photo/$page.js` served `/feed/(.)photo` — a `(group)` is a
...
- a slot holds pages, layouts, templates, loading fallbacks, nested slots and
  one default — `$not-found.js`, `$route.js` or `$middleware.js` inside one is
  refused, because uf's slots do not carry those request-level answers of
  their own yet
