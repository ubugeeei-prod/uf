...
- a slot holds pages, layouts, nested slots and one default — a `$loading.js`,
  `$error.js`, `$not-found.js`, `$template.js`, `$route.js` or
  `$middleware.js` inside one is refused, because uf's slots do not carry
