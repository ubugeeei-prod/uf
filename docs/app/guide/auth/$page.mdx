...
**What you will be able to do:** add sign-in against an OAuth provider, wire the
four handlers, read who is signed in, and replace the session store.

**What you need first:** route handlers — [Answering requests](/guide/routing/requests) — and a
provider that issues you an OAuth client.
...
Each handler is a plain `Request` → `Response`, which is what a `$route.js`
