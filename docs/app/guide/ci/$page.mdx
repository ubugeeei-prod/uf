...
On a Windows runner it uses the PowerShell installer and installs the Windows
release artifact. A matrix leg that cannot install uf fails rather than quietly
doing nothing, because a skipped platform still reports the matrix green.
