...
**What you will be able to do:** run uf in GitHub Actions, GitLab CI or
CircleCI, pin its version, install before you check, cache what is safe to
cache, and read the result from a job.

**What you need first:** a repository with a uf project in it — [Your first
project](/guide/project) — and [Tasks](/guide/tasks), if the pipeline and a
laptop should run the same list.
...
| --- | --- |
| GitHub Actions | [`ci-setup/github-actions/action.yml`](https://github.com/ubugeeei-prod/uf/blob/main/ci-setup/github-actions/action.yml) |
| GitLab CI | [`ci-setup/gitlab/uf.gitlab-ci.yml`](https://github.com/ubugeeei-prod/uf/blob/main/ci-setup/gitlab/uf.gitlab-ci.yml) |
| CircleCI | [`ci-setup/circleci/orb.yml`](https://github.com/ubugeeei-prod/uf/blob/main/ci-setup/circleci/orb.yml) |

The examples pin a release published before the directory was renamed to
`ci-setup/`, so the action and GitLab URLs still name `integrations/` at that
tag. For a release containing the new directory, use `ci-setup/` in those URLs.
...
The shared action currently installs uf on macOS and Linux runners. It refuses
Windows runners; its steps have not yet been wired to the PowerShell installer.
For a Windows runner, follow the [installation guide](/guide/install) to install
the native Windows release directly, or run the Linux setup under WSL2.
...
A pipeline is the second place a check runs; the first is the commit.
`uf prepare` is the pre-commit command — the staged files, the generated types,
whatever [`staged`](/reference/config#staged) in `uf.config.js` names, then
`uf lint` and `uf fmt --check` — and one command wires it into git:
...
Commit `.githooks/pre-commit`. Git keeps `core.hooksPath` in `.git/config`,
which is not versioned, so each clone runs `uf prepare --install-hooks` once —
uf runs no `postinstall` script to do it on anyone's behalf. A clone without
`uf` on `PATH` has its commit refused, with the way to skip the check once on
purpose: `git commit --no-verify`.

The run checks what is staged. A file that is half staged is checked as it will
be committed, and its unstaged half is put back afterwards. A staged task that
fixes a fully staged file has its fix staged with it; one that would have to fix
a half-staged file stops the commit instead, so the fix is made by hand to the
half that is being committed. CI runs the same checks over the whole tree, which
is what still catches a commit made with `--no-verify`.
...
`curl -fsSL https://setup.uniflowed.dev | sh` (or
`irm https://setup.uniflowed.dev/install.ps1 | iex` on Windows) followed by
adding the installer's bin directory to `PATH` is the whole of the first two,
and everything on this page from [Install before you check](#install-before-you-check)
onwards applies to a system with no file here exactly as it applies to the three
that have one: they are properties of uf, not of the runner.
...
lockfile CI holds a project to. [Agents](/guide/agents) runs the same checks from
a coding agent over `uf mcp`.
