...
This is the HTML half of what comes off the wire. The same document carries the
Flight payload its tree was rendered from, and the browser hydrates that payload
rather than rendering the route's modules again —
[Server Components](/guide/server-components#the-current-contract) has the rest.
