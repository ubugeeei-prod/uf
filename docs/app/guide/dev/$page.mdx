...
**What you will be able to do:** run the dev server and the production build,
know what uf hands Vite and what it leaves alone, work through a hydration
failure, and reach past uf to Vite when you need to.

**What you need first:** a project with its dependencies installed — [Your first
...
your `$middleware.js`, then a server action, then a route handler, then the
renderer — for every method and whatever the client said it would accept. So a
`$route.js` answers a browser here exactly as it does under `uf start`, and a
...
A `$loading.js` and a `$error.js` are decisions about *part of the page*,
...
sit between a pair of `<span hidden data-uf-boundary>` elements, which carry no
content and are ordinary siblings of the nodes between them:

```html
<div id="uf-root">
  <nav class="masthead">…</nav>
  <span hidden data-uf-boundary="suspense:0" data-uf-boundary-edge="open"></span>
  <article class="post">…</article>
  <span hidden data-uf-boundary="suspense:0" data-uf-boundary-edge="close"></span>
...
A `$loading.js` is a promise about *time*: the shell goes out now and this
part of the page follows when it is ready. Whether it kept that promise is the
one thing a browser makes hard to see — the fallback may have flashed past in
40 ms, or never rendered at all because the page resolved first, and the
finished document looks identical either way.

So `uf dev` says what left. When a route streams, your terminal names each
boundary that arrived after the shell, how long after, how big it was, and which
part of the tree it built:
...
`p.skeleton` is what your `$loading.js` rendered and `article.post` is what
replaced it, both read off the wire rather than out of the route table. `B:0` is
React's own name for the boundary, printed rather than translated: React numbers
boundaries as it meets them, uf numbers a route's fallbacks, and a `<Suspense>` a
component wrote itself is in the first list and not the second — so the two
cannot honestly be joined.

Like the boundary map, it is quiet: it speaks the first time a route streams and
whenever the *shape* of that stream changes — a fallback added, a boundary that
stopped streaming — and says nothing when only the milliseconds moved, which
would be every reload. A page with nothing suspended in it says nothing at all,
because there is no order to report about a document that arrived whole.

This is the half of the picture that comes off the wire. uf has no Flight payload
yet — the browser re-renders the matched tree from the same modules the server
used, which is why the client/server split is route-level (see
[Server Components](/guide/server-components)) — and when there is one, this is
...
`--analyze` answers the question that comes next, which is why. For every route
it lists each module the route ships to the browser and runs on the server,
what each weighs, and the chain of imports that put it there, such as
`app/chart/$page.js → app/chart/Plot.js → node_modules/chart.js`. It writes
`.uf/build/meta/uf-bundle-analysis.html`, a page you open from disk, and the
same analysis as JSON beside it. The [command reference](/reference/cli) has
the schema.
...
[Formatting and linting](/guide/format) is next: `uf fmt` and `uf lint`, over
the same syntax tree the build reads.
