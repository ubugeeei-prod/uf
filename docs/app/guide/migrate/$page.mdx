...
| `@slot/` | `@slot/` — a parallel route: the segment's own layout receives it as a prop beside `children`, and the slot's pages are matched against the same URL. Slot-local `loading` and `template` files compose inside the slot; per-slot `error` and `not-found` are still refused ([#267](https://github.com/ubugeeei-prod/uf/issues/267)) |
