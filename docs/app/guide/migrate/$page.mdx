...
This page is for a reader moving an application they already have. Whatever it
is built with, read [the three questions](#three-questions-that-mean-not-yet)
below first — each is a wall rather than a speed bump — and then the row for
what you have:
...
| Next.js | [What uf does not do](/guide/scope), then [From Next.js](#from-nextjs) |
| Expo | [Compared with Expo](/guide/react-native#compared-with-expo), in the React Native guide |
| TypeScript, in any of the above | [Flow and TypeScript](/guide/typescript), then [Converting the types](#converting-the-types) |

Parts of this page predate the releases that closed the gaps they describe, and
some of the issues it names as open walls are closed. Until
[#954](https://github.com/ubugeeei-prod/uf/issues/954) rechecks it against the
current release, treat an issue link here as something to check rather than an
answer.
...
| `unknown` | `mixed` |
| `readonly x: T` | `readonly x: T`, unchanged — Flow's older `+x: T` is an error under `uf check` |
...
| `@slot/` | `@slot/` — a parallel route: the segment's own layout receives it as a prop beside `children`, and the slot's pages are matched against the same URL. Slot-local `loading`, `template` and browser `error` files compose inside the slot; per-slot `not-found` is still refused ([#267](https://github.com/ubugeeei-prod/uf/issues/267)) |
| `(.)folder/` | `@slot/(.)folder/` — an intercepting route, inside the slot it renders into: a client navigation from a page the slot is on shows it there, and a reload renders the page the URL names. Outside a slot it is refused, and so is one whose URL has no page of its own; see [Intercepting routes](/guide/routing#intercepting-routes) |
...
| `loading.tsx` | `$loading.js` — a `<Suspense>` boundary per segment, nested; a slow page or deferred loader value streams the fallback before the finished segment |
