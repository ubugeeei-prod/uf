...
[Rendering modes](/guide/rendering) for where a document comes from. Server
Components also need React 19.3 or newer, with `react-dom` and
`react-server-dom-parcel` from the same release.

`@uniflowed/router` itself installs beside React 19.2.3, the React that Expo SDK
57 and React Native 0.87 ship, and lists `react-dom` and
`react-server-dom-parcel` as optional peers. A native app, or an application
rendered from its modules, does not need them. On a React older than 19.3,
`uf dev` and `uf build` stop before they render and name the React they found,
and the router's Server Components entries (`@uniflowed/router/rsc`,
`@uniflowed/router/rsc/client` and `@uniflowed/router/rsc/ssr`) refuse to start
...
| Diagnostics | invalid directives, server-only imports in client graphs, client APIs in server modules, and project hooks that reach one are named | a hook from a package the analysis cannot read is a warning, not a verdict |
...
| Navigation | a navigation fetches the next route's payload at `<route>/__uf.flight`; a prerendered route has that file beside its `index.html`; an intercepted navigation asks for the page underneath with the interception in its slot | no known gap |
| Server actions | exported async functions in `"use server"` modules are callable by id, from an import or passed down by a Server Component as a prop, bound or not ([#1359](https://github.com/ubugeeei-prod/uf/issues/1359)) | inline action references ([#1360](https://github.com/ubugeeei-prod/uf/issues/1360)); a form works before hydration with an imported action, not with one passed as a prop ([Server actions](/guide/server-actions#what-is-not-here)) |
...
talking rather than React's: `npm/router/client.js` hydrates by
...
| Rule | What it means |
| --- | --- |
| `rsc/server-only-import-in-client` | A module the client graph reaches imports `@uniflowed/db`, `@uniflowed/server`, `server-only`, a subpath of one of those, or a `*.server.js` file. The message names the chain of imports from the client boundary down to that module — `` `app/Counter.js` → `app/format.js` → `app/session.js` `` — because the import to cut is on that path, and rarely in the module that did the importing |
...
That last row is the only thing in the analysis that is not a verdict. It is
a warning because it is a statement that the analysis *stopped*. A hook you
wrote in the project is followed into its body: one that reaches `useState` or
another client-only API is `rsc/client-only-hook-in-server`, naming the module
that defines it, whatever it was imported as
([#388](https://github.com/ubugeeei-prod/uf/issues/388)). A hook whose body the
analysis cannot read, such as `useRoute` from a package that is not on the name
lists, gets the warning instead. Asking the question is more honest than
staying silent, because silence reads as "checked and fine", which is the one
thing it is not.
...
**All of your server code runs in the `rsc` graph**: pages, layouts, loaders,
server actions, route handlers (`$route.js`) and middleware (`$middleware.js`).
A module that any two of them import is one instance per process. What an
action or a `POST` handler writes to an in-memory store is what the next render
reads, including the page a form posted before hydration gets back. A
module-level cache, rate limiter or connection pool is likewise shared by
everything that imports it
([#1469](https://github.com/ubugeeei-prod/uf/issues/1469),
[#1487](https://github.com/ubugeeei-prod/uf/issues/1487)). One instance per
*process* only: a deployment with more than one process still needs shared
state outside the process.

The cost is that a route handler and a middleware resolve their imports under
the `react-server` condition, the way the pages do. A package that ships a
different build under that condition gives them that build. In particular,
`react-dom/server` does not load there, so a handler cannot call
`renderToString` to render an email. Do that rendering outside the handler, for
example in a service that the handler calls.
...
- **`$error.js` is a client module.** An error boundary catches a throw while
  ...
  without `"use client"` fails `uf build` (and `uf dev`'s route table) with its
  file named, before any page can throw.
...
- **A form that posts before hydration, with an action passed as a prop.**
  Passing an action down works once the page has hydrated. A form that has to
  work before that imports the action instead
  ([Server actions](/guide/server-actions#what-is-not-here)).
- **Inline `"use server"` closures**, which have no export name to reference
  ([#1360](https://github.com/ubugeeei-prod/uf/issues/1360)).

[What uf does not do](/guide/scope) lists these beside every other gap.

## Where to go next

[Server actions](/guide/server-actions) is next: the other half of the server
boundary, the functions a form or a transition calls on the server.
[Async React](/guide/async-react) is what a reader sees while that work is
unfinished.
