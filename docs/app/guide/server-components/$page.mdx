...
graph decided, see which routes keep their page out of the client bundle, and
know what uf reports and where `uf lint` fits.
...
The RSC work is deliberately documented as a contract with a missing half, not
as a finished Next.js-equivalent implementation.

| Area | Today | Missing before the claim changes |
| --- | --- | --- |
...
| Route-level split | a route with no reachable client boundary can be removed from the client route table | splitting a Server Component above a boundary out of the client bundle |
| HTML streaming | Suspense boundaries stream through the document renderer | client-side reconstruction of the streamed Server Component tree |
| Server actions | exported async functions in `"use server"` modules are callable by id | inline action references and native form progressive enhancement |
| Flight payload | not implemented | the payload format, client references, chunk ids and a second React environment |

That table is the difference between "RSC analysis exists" and "uf has React
Server Components as a full app model". The first is true now. The second waits
on the Flight payload and the bundler environment that can build React under
the `react-server` condition separately from the client.
