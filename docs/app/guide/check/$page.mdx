...
The lint half runs on every core. Inference runs on one thread. What the React
Compiler reports about each module — nearly all of what the lint half costs — is
cached under `.uf/cache/lint/`, keyed on the module's path and text, the React
rules' switches and the `uf` binary, so `uf lint` and `uf check` hand only the
modules that changed to the compiler again. There is no watch
...
go to definition and member completion — see [IDE setup](/guide/editors).
...
that a different error on the same line is still reported. A suppression that
suppresses nothing is reported as a warning, "Unused suppression comment.", on
the comment itself, as Flow reports it. It is a warning rather than an error, so
a stale comment does not fail a run. Delete it, or correct its code: an error
code Flow has since renamed is the usual reason a working suppression stops
matching.
...
the same syntax tree. [uf in CI](/guide/ci) runs `uf check` after `uf install`,
and [Flow and TypeScript](/guide/typescript) is the argument for the type system
this command runs.
