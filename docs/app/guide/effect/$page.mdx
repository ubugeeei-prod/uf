...
typed failure. `trySync` and `tryPromise` name a failure through an explicit
error mapper:
...
Use `trySync` when a synchronous API throws an expected input error. It preserves
its original value for the error mapper and works with both `runSync` and
`runPromise`; `sync` treats throws as defects. A mapper that throws is also a
defect. Both callbacks run only when the effect executes.

```js
import { trySync } from "@uniflowed/effect";
...
Compose this with `ensuring` when the operation owns cleanup, such as rolling back
a synchronous database transaction. Cleanup runs before the original typed
failure is returned to the caller.

## Where to go next

[Headless components](/guide/ui) is next: keyboard maps, ARIA and composition,
with no styles attached. [Effects](/reference/effect), in the reference, has
every export.
