...
A `Schedule` is plain data — eight shapes, no builder:
...
`recurs`, `spaced`, `exponential`, `fibonacci`, `upTo`, `intersect`, `union` and
`maxDelay`. `factorPercent` is an integer percentage rather than a float,
because 1.5 written as `150` survives a round trip through a JSON config file
without becoming `1.4999999999999998`.

`scheduleDelay(schedule, attempt)` from `@uniflowed/effect/schedule` is the whole
runtime: it answers the delay before an attempt, or `null` when the schedule is
finished. It is arithmetic with no failure mode, which is what lets a schedule
say *when* to try again and never *whether*.
...
| `fork` is scoped to its parent | `fork` is `forkDaemon` | Structured children are what `all`, `race` and `timeout` use internally |
...
**Not implemented**, quoting the package: `Ref`, `Deferred`, `Queue`, `Hub`,
`Semaphore` and STM; streams; a fiber scheduler of its own — this runs on the
host's microtask queue and its `sleep` is `setTimeout`; tracing, spans, metrics
and the logging layer; `Layer` memoisation, so a layer passed to two `provide`s
is built twice; and a typed defect channel.
