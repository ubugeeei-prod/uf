...
1. `pm.packageManager` in `uf.config.js`;
2. the `packageManager` field in `package.json`, such as `"bun@1.3.13"`;
3. a lockfile in the project root — `bun.lock`, `bun.lockb`, `pnpm-lock.yaml`,
   `yarn.lock`, `package-lock.json` or `npm-shrinkwrap.json`, in that order;
4. a workspace root above the project that says one of the above;
5. npm.
...
([#940](https://github.com/ubugeeei-prod/uf/issues/940)), `pm.packageManager` is
