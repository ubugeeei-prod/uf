...
name only, Yarn 2+ by path only, and uf refuses the other form with the one to
use. `uf dedupe` collapses the versions your ranges allow to be one.
...
| `uf install --prod` | `npm install --omit=dev` | `pnpm install --prod` | `yarn install --production` | `yarn workspaces focus --all --production` | `bun install --production` |
...
| `uf link NAME` | `npm link` | `pnpm link` | `yarn link` | refused | `bun link` |
