...
manager `--ignore-scripts`. Yarn 2+ rejects that flag on every command as an
unsupported option, so it is given the setting Yarn reads instead,
`YARN_ENABLE_SCRIPTS=false`, and `pnpm remove`, `pnpm link` and
`pnpm patch-commit`, which do not accept the flag either, are given
`--config.ignore-scripts=true`. Some dependencies genuinely need
to build, and
...
install-time lifecycle scripts and every dependency's — and is the one switch
that does. A script no install runs, such as `start` or `ios`, is never refused.
...
In a workspace, `--filter` chooses the members `uf add`, `uf remove` and
`uf update` change, with the selectors `uf run --filter` takes, and `-w` chooses
the workspace root from anywhere inside it:

<figure className="terminal">

```
$ uf add zod --filter api --filter web

  manager    pnpm
  chosen by  pnpm-lock.yaml
  command    pnpm add --ignore-scripts zod  (api)
  command    pnpm add --ignore-scripts zod  (web)
  lockfile   pnpm-lock.yaml · 212 packages · 96.10 kB

  manifest

       workspace  field         package  range
    +  api        dependencies  zod      ^4.1.8
    +  web        dependencies  zod      ^4.1.8

✓ 2 packages recorded in dependencies in 2.41s
```

</figure>

uf runs the manager in each member's directory, which npm, pnpm, both Yarns and
Bun all read as "this member", and the lockfile they write is the workspace
root's. A selector that picks nothing is refused with the list of members.
Yarn 2+ cannot update one member — `yarn up` moves a package everywhere — so
`uf update --filter` is refused there.

`uf link ../ui` links a package you are developing into the project, and `uf link`
inside it makes it linkable by name from other projects. Yarn 1 and Bun link by
name only, Yarn 2+ and pnpm 12 by path only, and the other form is refused with
the one to use. What `uf link` reports is read from `node_modules`, because npm,
Yarn 1 and Bun link without writing the manifest or the lockfile: the report names
the package and where its link leads, and a manager that exits 0 without linking
anything is a failure, not `already up to date`. `uf unlink` undoes either
direction. It takes a link out of the project and installs what the manifest
declares in its place, or removes what `uf link` registered. When there is
nothing to unlink, it says what it found and exits 0. `uf dedupe` collapses the
versions your ranges allow to be one, and `uf dedupe --check` names what it
would collapse, changes nothing, and exits non-zero when there is anything —
...
| `uf install --prod` | `npm install --omit=dev` | `pnpm install --prod` | `yarn install --production` | `yarn workspaces focus --all --production` | `bun install --omit=dev` |
...
| `uf dedupe` | `npm dedupe` | `pnpm dedupe` | refused | `yarn dedupe` | refused |
| `uf dedupe --check` | `npm dedupe --dry-run --json` | `pnpm dedupe --check` | refused | `yarn dedupe --check` | refused |
| `uf link DIR` | `npm link` | `pnpm link` | refused | `yarn link` | refused |
| `uf link NAME` | `npm link` | `pnpm link` on pnpm 10; pnpm 12 refuses it | `yarn link` | refused | `bun link` |
| `uf unlink NAME` | `npm uninstall --no-save` | `pnpm unlink` | `yarn unlink` | `yarn unlink` | uf removes the link |
...
| `uf info` | `npm view` | `pnpm view` | `yarn info` | `yarn npm info` | `bun info` |
| `uf search` | `npm search` | `pnpm search` | refused | refused | refused |
| `uf patch` | refused | `pnpm patch` | refused | `yarn patch` | refused |

`uf info react` prints the registry's metadata for a package, and
`uf info react version` one field of it.
...
package name or its path, and `uf add react --filter web --filter 'api...'` in
every member the selectors pick — see [Adding and removing](#adding-and-removing).
A workspace's members are what `package.json#workspaces`, `pnpm-workspace.yaml`
and the members' own `uf.config.js` say they are. `uf catalog` lists the packages more than one manifest
