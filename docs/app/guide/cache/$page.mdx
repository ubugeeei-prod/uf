...
**What you will be able to do:** turn on the route and fetch caches per route
and per request, set revalidation windows, invalidate by tag or by path, and
choose where the cache lives.

**What you need first:** routes that render on a server — [Rendering
modes](/guide/rendering) — and [Routing](/guide/routing) for loaders.
