...
- A regenerated page is **never unservable on a clock**. Past its lifetime a
  ...
  wait.
- `revalidateTag` and `revalidatePath` expire it like any other entry, and a
  process never starts a page from the build twice, so an invalidated page is
  rendered rather than served from the build's older copy.
...
- **An invalidation a restart remembers.** A process keeps "this page was
  already started from the build" in memory. One that restarts after
  `revalidateTag` expired a regenerated page, and before anything regenerated
  it, finds nothing in the store and starts the page from the build's copy
  again — so that copy can be served until the page's lifetime, counted from the
  build, has passed. Recording invalidations in the store would close the
  window, and no provider uf ships does.
