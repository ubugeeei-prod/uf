...
**What you will be able to do:** declare atoms, read and write them inside and
outside React, derive, compose and persist state, and know where
`@uniflowed/state` parts company with Jotai.

**What you need first:** a project — [Your first project](/guide/project) — and
[Flow, the modern parts](/guide/flow) for `hook`.
...
[#521](https://github.com/ubugeeei-prod/uf/issues/521), and each named the
...
[the layer below](#the-layer-below) directly, with no store in the way.
...
point of the example. `packages/state/state.test.js` is where the behaviour above
is pinned, with that setup written out — `uf test packages/state/state.test.js`, and
`cell.test.js` for the layer below.

## Where to go next

[Styling and content](/guide/styling) is next: CSS, StyleX, tokens and dark
mode, and pages written in Markdown and MDX.
