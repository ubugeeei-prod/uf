...
The `@uniflowed/*` packages are **not on npm yet**, so `uf install` in a fresh
project cannot fetch them; see
[publishing status](/reference/packages). This application was written and run
inside uf's own repository, where the packages resolve through the npm
workspace. Everything else here — the commands, the code, the output — is what
a published toolchain gives you, and the day the packages land, the only step
that changes is the one that installs them.
...
/// Titles the reader typed into the form on this page. Client-side only: this
/// is what the form writes, and it is thrown away on reload.
export const suggestions = atom<$ReadOnlyArray<string>>([]);

export const suggest = action<string>((get, set, title) => {
  set(suggestions, [...get(suggestions), title]);
});
```
...
you registered — `errors["profile.city"]`, not `errors.profile.city`. Field
paths are `string` and values are `mixed`, because Flow has no template-literal
types and a `FieldPath<T>` that is really `string` would be a type pretending to
check something. And reading `formState` where you called `useForm` costs one
...
`dist/` is static files, and any static host serves them. None of the three JSON
files uf writes beside the HTML is needed at run time: `uf-build-manifest.json`,
`uf-bundle-report.json` and `uf-rsc-manifest.json` are for you and for tooling.
There is no deployment guide on this site yet — the portable-output work is
[issue #245](https://github.com/ubugeeei-prod/uf/issues/245) and the runtime
coverage is [issue #246](https://github.com/ubugeeei-prod/uf/issues/246).
