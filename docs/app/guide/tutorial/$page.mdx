...
✓ created 10 files in /private/tmp/claude-501/cap/reading-list
```
...
[Deploy a web build](/guide/deploy) is the guide for what happens after this:
`uf preview`, `uf start`, `uf build --adapter` for one of seven targets, or
`uf build --compile` for a single executable. No server target's output has
been deployed to its real platform by uf's own pipeline yet
([#956](https://github.com/ubugeeei-prod/uf/issues/956)).
