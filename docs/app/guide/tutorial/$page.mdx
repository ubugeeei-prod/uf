...
description: "One application end to end: routes, data, state, a form, tests, the build."
---
...
**What you will be able to do:** build one application from `uf new` to a
directory you can upload — routes, a layout, a loader, shared state, an
accessible tab list, a validated form and its tests — and read what each command
printed.

**What you need first:** uf [installed](/guide/install), and [Your first
project](/guide/project) for what the scaffold writes. [Before you
start](#before-you-start), below, has the rest.
...
<div className="command"><code>uf new reading-list</code></div>
...
`app/Counter.js`, `app/useCounter.js` and `app/$page.test.js` — and start.
...
A page is `$page.js`, and a page that needs data exports a `loader` that runs
before it renders:
...
`app/articles/[slug]/$page.js` serves `/articles/one-parse`:
...
Without it, a dynamic route gets no file: `uf build` lists it under `answered by
a server`, and a static host would answer it with a 404. On `0.0.0-alpha.5` the
build skipped it without saying so, which was the one thing on this page that
...
`app/$layout.js` renders the document and receives the page as `children`:
...
scrolls the page. Each of those is an assertion in `packages/ui/ui.test.js`,
...
Those two Node warnings came from installing the DOM, not from your test, and
`uf test` no longer prints them:
[#308](https://github.com/ubugeeei-prod/uf/issues/308) is closed, and
`packages/react-testing/dom-storage.test.js` asserts a component test prints no
`--localstorage-file` warning. They are left in this transcript because it was
taken on `0.0.0-alpha.5`, and a clean one would have to be invented.
...
───────────────────────
  ...
  shipped
    ...
    assets/$page-CCpsAGfo.js.map                source-map   58.84 kB  213.07 kB
    assets/state-C5bwrBVn.js.map                   source-map   29.72 kB   98.43 kB
    assets/$page-CCpsAGfo.js                    javascript   10.10 kB   28.65 kB
    assets/state-C5bwrBVn.js                       javascript    2.58 kB    6.56 kB
    assets/articles-DzlNJoIc.js.map                source-map      911 B    2.00 kB
    assets/$page-Dk6TuWlb.js.map                source-map      803 B    1.97 kB
    index.html                                     html            801 B    1.95 kB
    assets/$layout-DVv7YWPk.js.map              source-map      743 B    1.92 kB
    assets/$layout-DVv7YWPk.js                  javascript      522 B      914 B
    assets/$page-Dk6TuWlb.js                    javascript      522 B      933 B
...
`@uniflowed/state`. On `0.0.0-alpha.5` they named no file, no line and no
function, so there was nothing an application author could do with them; a
React Compiler finding names its file and its function now
([#307](https://github.com/ubugeeei-prod/uf/issues/307)). They are the only
...
There is no guide to deploying yet
([#955](https://github.com/ubugeeei-prod/uf/issues/955)). What exists is in
[Targets](/guide/targets) and the
[command reference](/reference/cli#serving-a-build): `uf build --adapter`
writes a directory for one of seven targets — `node`, `bun`, `deno`,
`container`, `edge` for Cloudflare Workers, `serverless` for AWS Lambda, and
`static` — and `uf build --compile` writes a single executable. No server
target's output has been deployed to its real platform by uf's own pipeline
yet ([#956](https://github.com/ubugeeei-prod/uf/issues/956)).
...
Four things were found while writing this page, and left visible rather than
written around. Three have been fixed since: the stylesheet `stylex.create`
compiles now reaches `dist/`
([#306](https://github.com/ubugeeei-prod/uf/issues/306)), a React Compiler
finding names its file and its function
([#307](https://github.com/ubugeeei-prod/uf/issues/307)), and a component test
no longer prints Node's `--localstorage-file` warning
([#308](https://github.com/ubugeeei-prod/uf/issues/308)). One still does not
work:
...
That is the end of Start. [Build an app](/guide/build-an-app) is next: one guide
per thing an application does, starting with the Flow syntax and the route table
this application leaned on. [Testing](/guide/testing) is watch mode, and where
the runner stands against Bun.
