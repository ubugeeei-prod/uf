...
Every part spreads whatever you pass onto its element, so `className`, `id`,
`data-*` and `aria-*` all arrive. `className` is the bridge — a DOM prop, not a
type from a styling library — which is what lets uf's own presets style these
without the package knowing they exist. [Styling](/guide/styling) is that half.
...
The difference is one key, and it is the reason they are two components rather
than one with a prop:

| Key | Checkbox | Switch |
| --- | --- | --- |
| `Space` | Toggles | Toggles |
| `Enter` | **Left to the form** — it submits | Toggles |

From `checkbox.js`: *"That is the difference between a control that answers a
question and one that operates a thing."* The test names are "toggles a checkbox
on Space and leaves Enter to the form" and "toggles a switch on Space and on
Enter".

`Checkbox` also takes `indeterminate`, which reports `aria-checked="mixed"`, and
clicking a mixed checkbox reports `true` rather than the opposite of a boolean
nobody set.
