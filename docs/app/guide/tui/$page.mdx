...
and selection, and test it without a terminal.
...
`TextTable`) and OpenTUI's remaining application APIs — notifications, audio,
...
The next selection-shaped gap is *item* selection — `Select` and `TabSelect`,
which are components rather than a renderer capability, and which is why
`uf inspect` reports text selection as implemented and neither of them as
existing. Repeated-press gestures that widen a selection to a word or line are
also still absent.
