...
The frame above is not a screenshot. `packages/tui/tui.test.js` renders the
...
`packages/tui/tui.test.js`, so this half of the table cannot go stale without a
...
That is counted rather than claimed. `packages/tui/tui.test.js` builds the
