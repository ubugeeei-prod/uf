...
- `errors()` lists the exceptions the application threw instead of answering.
  A host reports those and answers with a bare `500`; `fetch` returns the
  `500`, and this is the report, so a test can assert on why a request failed
  without the reason ever being in a response.
...
The second and third cases are ones a unit test cannot make: an action or a
route handler writes, and the next render of the page reads what it wrote. That
holds because server actions, route handlers and middleware run in the same
module graph as the pages
...
| | Status |
| --- | --- |
| `withRequest`, `callAction` (both doors), `serverReferences`, `buildApp`, `openBuild` | **Experimental.** Implemented and tested (`npm/router/testing.test.js`); the names may still change |
