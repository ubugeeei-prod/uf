...
"RSC and browser testing" job type-checks it and runs its tests. Each sample on
this page is checked to be a verbatim excerpt of the file it names
(`tests/library/docs-sources.test.js`), so the page cannot drift from what
passes.
...
| `"redirect"` | called `redirect()`; `to` and `permanent` | `500`, see below |
| `"not-found"`, `"unauthorized"`, `"forbidden"` | threw the router's error | `500` |
...
**A hydrated call cannot redirect yet.** A form posted *before* hydration
answers `redirect()` with a `303`. Once the page has hydrated, the same action
is called through the JSON door, and that door answers every routing error with
the fixed `500`. `callAction` reports both halves, `kind: "redirect"` and
`response.status === 500`, so a test documents the gap rather than hiding it
([#1470](https://github.com/ubugeeei-prod/uf/issues/1470)). Pass
`door: "form"` to send the call the way a browser does before hydration. The
last argument is then the `FormData`, the ones before it are the arguments the
form carries bound, and the answer is the `303`.
...
Each reference is what the browser's is. It goes through `callAction`,
answers with the decoded value, and rejects with the `ServerActionError` a
browser's reference throws when the endpoint does not answer `200`. `init` can
...
<div className="command"><code>buildApp({ root })</code></div>

```js source=crates/uf_cli/tests/fixtures/rsc-test-app/tests/notes-app.test.js
// @flow
...
**One case is skipped, and it is a bug.** In a built application, the server
actions run in a different module graph from the pages. A module both import,
like this fixture's in-memory store, is evaluated twice, so the page never sees
the action's write
([#1469](https://github.com/ubugeeei-prod/uf/issues/1469)). The case is in the
file with `it.skipBecause` naming the issue, and it passes once that is fixed.
An application whose state lives outside the process, such as a database or an
upstream service, is not affected.
...
| An action and the page it changes sharing module state | **Bug**, [#1469](https://github.com/ubugeeei-prod/uf/issues/1469) |
| `redirect()` and the other routing errors from a hydrated action call | **Bug**, [#1470](https://github.com/ubugeeei-prod/uf/issues/1470); a `500` today |
