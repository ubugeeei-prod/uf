...
description: "component, hook, renders, match and enums — and what uf does with them."
---
...
**What you will be able to do:** write `component`, `hook`, `renders`, `match`
and `enum`, and know what uf turns each one into before Vite sees it.

**What you need first:** a project — [Your first project](/guide/project) — and
React you already know. [Why Flow](/guide/why) is the argument for this syntax,
...
const label = match (status) {
  "idle" => "Ready",
  "loading" => "Working…",
  {kind: "failed", const code} if (code >= 500) => "Server error",
  {kind: "failed", ...} => "Failed",
};
```
...
[Routing](/guide/routing) is next: files under `app/` as the route table, and
the layouts, loaders and boundaries a page is rendered inside.
