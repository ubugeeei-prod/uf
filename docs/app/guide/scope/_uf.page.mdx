...
| **Every implemented package on npm.** Seventeen names go out on each `uf@*` tag — the closure a project needs, plus the test runner. Thirteen more are implemented and waiting in `tools/release/pending-packages.txt`, `@uniflowed/state` and `@uniflowed/effect` among them | Those thirteen resolve only through this repository's workspace, which is how this site imports them. `@uniflowed/std` is the newest and the odd one: its root is a declaration surface and six of its subpaths are code, so the per-package check that finds the others cannot see it — it is on the list by decision | [#210](https://github.com/ubugeeei-prod/uf/issues/210) |
