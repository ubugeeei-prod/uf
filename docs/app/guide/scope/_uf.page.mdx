...
| **Every implemented package on npm.** Twenty-nine names go out on each `uf@*` tag — the closure a project needs, plus the test runner. One implemented package remains in `tools/release/pending-packages.txt`: `@uniflowed/std`, whose root is a declaration surface while six subpaths are code, so the per-package check that finds the others cannot see it — it is on the list by decision | `@uniflowed/std` resolves only through this repository's workspace, which is how this site imports it. Everything else in the published closure has a registry name at the current prerelease | [#710](https://github.com/ubugeeei-prod/uf/issues/710) |
