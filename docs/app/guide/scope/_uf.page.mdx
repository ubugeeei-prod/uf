...
| **React Server Components.** `"use client"` and `"use server"` are scanned, graphed and manifested; nothing that *builds* reads the result. `uf dev` reports the contract violations and `uf build` fails on them, and neither splits anything | Every page module ships to the browser, and no server action is callable. The client-only check matches API names, so a hook that wraps one is reported as unclassified rather than resolved | [#252](https://github.com/ubugeeei-prod/uf/issues/252), [#388](https://github.com/ubugeeei-prod/uf/issues/388) |
...
| **`.env` files.** `env.files` and `env.active` have no reader, `uf env use` writes a file nothing reads, and the Vite driver sets `envDir: false` | A project can turn Vite's own loader back on with `vite: { envDir: "." }` | [#259](https://github.com/ubugeeei-prod/uf/issues/259) |
