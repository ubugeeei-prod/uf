...
| **Every implemented package on npm.** Twenty-nine names go out on each `uf@*` tag — the closure a project needs, plus the test runner. Two implemented packages remain in `tools/release/pending-packages.txt`: `@uniflowed/temporal`, whose front door is implemented and still needs its first npm name, and `@uniflowed/std`, whose root is a declaration surface while six subpaths are code, so the per-package check that finds the others cannot see it — both are on the list by decision | `@uniflowed/temporal` and `@uniflowed/std` resolve only through this repository's workspace, which is how this site imports them. Everything else in the published closure has a registry name at the current prerelease | [#560](https://github.com/ubugeeei-prod/uf/issues/560), [#710](https://github.com/ubugeeei-prod/uf/issues/710) |
