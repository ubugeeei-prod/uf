...
| Deployment maturity | Next.js | seven adapters and `--compile` are implemented, but no server adapter has been deployed in anger; `serverless` buffers streamed documents, and every hosted target still needs real-world proof ([#391](https://github.com/ubugeeei-prod/uf/issues/391), [Commands](/reference/cli#uf-build---adapter-target)) |
...
| Runtime coverage | Bun, Next.js | Node.js and Bun are the comfortable local hosts; Deno, edge and serverless are deploy artefacts with target limits, not the same complete day-to-day runtime story ([#246](https://github.com/ubugeeei-prod/uf/issues/246), [Commands](/reference/cli#uf-build---adapter-target)) |
...
| ISR, caching, revalidation | yes | route and fetch caches are opt-in; `ppr`, `isr`, action caches and data caches are refused or planned rather than silently accepted ([Caching](/guide/cache), [Rendering modes](/guide/rendering)) |
...
And a framework whose server is optional at the route level, which is a feature
exactly when part of the application really is files.
