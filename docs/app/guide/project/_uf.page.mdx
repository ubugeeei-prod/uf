...
<div className="command"><code>uf create app react my-site</code></div>

`react` is the template and `my-site` is the directory. Both are positional, and
the template is not optional today even though `react` is the only one — a bare
`uf create app my-site` is rejected, because `my-site` is read as the template.
