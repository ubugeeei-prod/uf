...
description: "From an empty directory to a built site."
---
...
**What you will be able to do:** scaffold a project, run it, and know what each
of the nine files it writes is for — and which config files a React project
usually has and this one does not.

**What you need first:** uf [installed](/guide/install), with `uf info` naming a
JavaScript host.
...
Both take an optional template after that. `react` is the default, so it is
left out here, and `monorepo` writes an application and a library as workspace
packages of one repository, with one configuration for the checks and tasks
that build the library before the application. `--lib` on either scaffolds a
library instead. A template can also come from somewhere else —
`github:owner/repo#<commit>`, or a tarball with its `--integrity` — as long as
it is pinned to its exact bytes; [the command reference](/reference/cli) says
what is checked.
...
A page is `$page.js`, exported as `Page`:
...
[Editors](/guide/editors) is next: connect your editor to `uf lsp`, so lint
findings and formatting reach it while you write. [Build a reading
list](/guide/tutorial) comes after it, and builds one application end to end.
