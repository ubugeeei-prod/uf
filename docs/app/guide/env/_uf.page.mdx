...
Add the two `.local` names to `.gitignore` yourself; uf does not write it for
you. Vite's dev server refuses to serve `.env` and `.env.*` over HTTP whatever
...
2. The profile `uf env use <name>` recorded, in `.uniflowed/profile`. It is a
   local choice — put it in `.gitignore` — and it lasts until you change it.
...
**Only a name that starts with `VITE_`.** That value is substituted into the
JavaScript the browser downloads, so it is public: anyone who opens the site can
...
prefix is refused, because it would put every variable in the bundle.
