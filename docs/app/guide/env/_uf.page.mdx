...
* **It reloads by restarting, not in place.** `uf dev` watches the files the
  cascade selects and starts the server again when one of them changes, saying
  which file it was. A restart rather than a hot update is the honest
  granularity: a prefixed value reaches the browser by being substituted into
  the bundle, so a new one has to be substituted again and every module that
  read it has to be evaluated again. `uf build`, `uf preview` and `uf start`
  read the files once, which is all a command that ends has to do.
