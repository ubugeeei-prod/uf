...
`npm/stylex/stylex.test.js` renders the *styled* switch and holds the preset
against it — the classes land on the control, and it still reports
`aria-checked` — while the keys are `npm/ui/ui.test.js`'s, in *"toggles a
...
(`npm/stylex/stylex.test.js`), and the preset applied to a headless
component.
