...
description: "CSS, StyleX, tokens, dark mode, Markdown and MDX."
---
...
**What you will be able to do:** style with CSS, or with StyleX and its tokens,
write pages in Markdown and MDX with highlighted code, and serve public files.
...
`@uniflowed/stylex/tokens.stylex.js` exports `ufTokens`, 45 custom properties
...
become green without a rule in the preset changing. The shipped values are
neutral greys with one quiet cobalt accent, and light and dark are designed as
two palettes; [the design note](https://github.com/ubugeeei-prod/uf/blob/main/docs/ui-visual-language.md)
explains the choice.
...
| Shape | `radiusSm` (2px), `radiusMd` (4px), `radiusLg` (6px), `radiusPill`, `sizeControl` |
| Motion | `durationFast` (120ms), `durationBase` (200ms), `durationSlow` (280ms); `easing` (standard), `easingEnter` (decelerate), `easingExit` (accelerate), none of which overshoots. There are no shadow tokens: surfaces are separated by a 1px `border` and a change of background |
...
`packages/stylex/stylex.test.js` renders the *styled* switch and holds the preset
against it — the classes land on the control, and it still reports
`aria-checked` — while the keys are `packages/ui/ui.test.js`'s, in *"toggles a
...
A headless part says what state it is in: `aria-selected="true"` on the
selected tab, `aria-expanded="true"` on a trigger whose panel is showing,
`data-active="true"` on the option a select's keyboard cursor rests on. A
condition can select exactly that, so the style follows the part rather than a
prop the page has to keep in step with it:
...
<Tabs.Tab {...props(styles.tab)} value="billing">
  Billing
</Tabs.Tab>;
```

A tab the arrow keys selected is drawn selected, and the page never had to
learn which one it was. The select's cursor is the case with no other answer:
focus stays on the trigger, so no pseudo-class matches the option, and without
a highlight a keyboard user cannot see which option `Enter` will take.

The shape is narrow on purpose: an attribute selector inside `:is()` or
`:not()`, with an unquoted value — `[data-state=open]`, never
`[data-state="open"]`. A state is emitted after `:hover`, `:focus-visible`,
`:active` and the form states, so a pressed toggle stays pressed under the
pointer; `":is([aria-pressed=true]):hover"` is emitted after the state on its
own, which is how that toggle gets a hover colour of its own.
...
### What holds it to a built page

An application build writes the stylesheet it compiled, and links it from the
prerendered HTML together with the `:root` block that declares the tokens its
rules read. `crates/uf_cli/tests/vite_styles.rs` builds such an application and
checks the artefact rather than the pipeline: every class on the prerendered
page has a rule, every custom property a rule reads is declared, and
`ufAutoTheme`'s overrides sit inside the media query that makes them a dark
theme. Until [issue #306](https://github.com/ubugeeei-prod/uf/issues/306)
closed, the classes on a built page named rules that did not exist, and this
section said so.

The rest is tested where it lives: the compiler (`crates/uf_stylex`, including
the contrast suite above), the runtime merge in `props`
(`packages/stylex/stylex.test.js`), and the preset applied to a headless
component.
...
A `$page.mdx` is a page. Markdown is GitHub-flavoured, front matter is
...
This page is `app/guide/styling/$page.mdx`. The `.lede` paragraph above it
...
[UI components](/guide/ui) is next: `uf ui add` writes components styled with
these tokens, built on parts that own the keyboard, ARIA and composition.
