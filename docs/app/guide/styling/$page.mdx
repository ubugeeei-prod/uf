...
| Motion | `durationFast` (120ms), `durationBase` (160ms), `easing` (decelerating, no overshoot). There are no shadow tokens: surfaces are separated by a 1px `border` and a change of background |
...
[Images, fonts, icons and cards](/guide/assets) is next: the assets a page
refers to, resized, self-hosted and drawn at build time.
