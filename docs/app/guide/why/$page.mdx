---
title: "Why Flow · uf"
description: "What Flow says about React that nothing else can, and what it costs."
...
**What you will be able to do:** explain what Flow says about React that other
type systems cannot, what that costs, and why it takes a whole toolchain rather
than a plugin.

**What you need first:** React you already know. [What uf is](/guide) is the
toolchain argument this page sits beside.
...
written down: `readonly field` is covariant, `writeonly field` contravariant. A
read-only prop is expressible as a property of the type rather than as a
convention. These are the places where a type system quietly lies to you, and
Flow's answers are stricter.
...
[Flow and TypeScript](/guide/typescript) is next: the same argument against the
type system you probably write today, with the rows TypeScript wins. [Flow, the
modern parts](/guide/flow) is the syntax, with what uf turns each construct
into.
