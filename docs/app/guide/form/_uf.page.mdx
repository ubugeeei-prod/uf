...
```js
import { validatorResolver } from "@uniflowed/form";
...
Measured, typing six characters into one field, each component rendered alone:

| | Renders |
| --- | --- |
| `useForm` + `register`, value shown through `useWatch` | 2 |
| `useState` + a controlled `input` | 7 |

The two are the mount and the moment the form first becomes dirty; the next five
keystrokes cost nothing. React Hook Form gets that second render to zero with a
`Proxy` that records which `formState` keys a render read. That technique
depends on a render having happened — which is exactly what the React Compiler
is allowed to skip — so this package does not use it, and says so instead of
...
`tests/library/form.test.js` is where the behaviour above is pinned —
`uf test#library form.test.js`.
