---
title: "Benchmarks · uf"
description: "Every uf command timed beside Vite+, Next.js, Bun, Vitest, ESLint, Prettier, Biome, Flow and pnpm, with the machine and the versions."
---
...
| Vite | TypeScript and TSX, `index.html` and `src/main.tsx`, routes lazily imported, tests against `vite-plus/test` | `vp dev`, `vp build`, `vp test`, `vp lint`, `eslint .`, `biome lint .`, `tsc --noEmit` |
| Next.js | App Router: `app/layout.tsx`, `app/rNNN/page.tsx`, the toggles marked `"use client"` | `next dev`, `next build` |
| Vitest, Bun | the library modules and their tests against `vitest` and `bun:test` | `vitest run`, `bun test` |
...
| many small test files | 50 files × 20 cases × 2 assertions, from `tools/bench/testing/suite.js` | `uf test`, `bun test`, `vitest run` |
...
- **Cold** removes a tool's own caches before every run: `.uf`, `dist` and
  ...
  and Vitest. Prettier, ESLint, Biome and `tsc` keep no cache unless asked to,
...
its `bench-comparison` artifact, and commit its `results.json` as
`tools/bench/toolchain/baseline.json`. This page is rebuilt from the new file.
