...
**What you will be able to do:** read LCP, CLS, INP, TTFB and FCP in the
browser, send them where you choose, and know where uf's numbers differ from
`web-vitals`'s.

**What you need first:** a page to measure — [Your first
...
Reporting a metric as it becomes final is still an option, and is what to
reach for when you want the object rather than the summary:
...
That is the end of Build an app. [Targets](/guide/targets) is next: where the
application runs — a server or a static host, a single file, a phone, a
terminal.
