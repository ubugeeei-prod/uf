...
React Aria is the comparison set for this package: not because uf should copy
its API, but because it names the accessible behaviours product teams actually
need. This table is the audit line for #948.

| React Aria area | uf status | Tracking |
| --- | --- | --- |
| Interactions: press, hover, focus-visible, long press, move, keyboard and merged props | Shipped as the shared interaction layer used by the primitives above | Held by `packages/ui/interactions.test.js` |
| Modal and overlay surfaces | Shipped: `Dialog`, `AlertDialog`, `Sheet`, `Drawer`, `Popover`, `Tooltip`, `HoverCard`, `Menu`, `ContextMenu`, `Menubar` | Held by `packages/ui/ui.test.js` |
| Select, combobox and menu collections | Shipped for select-only and editable combobox flows, with option groups, typeahead and menu semantics | Held by the `Select`, `Combobox` and `Menu` blocks in `packages/ui/ui.test.js` |
| Standalone collection primitives | Shipped: `ListBox`, `GridList`, `Tree` and `TagGroup` with stable item keys, controlled selection, disabled keys, typeahead and optional fixed-height virtualization | [#1309](https://github.com/ubugeeei-prod/uf/issues/1309) |
| Calendar and single-date picker | Shipped: `Calendar` and `DatePicker` cover the single-date path | Held by the calendar and date-picker blocks in `packages/ui/ui.test.js` |
| Range and segmented date/time fields | Shipped: `RangeCalendar`, `DateRangePicker`, `DateField` and `TimeField` with segmented editing, bounds, unavailable dates and range announcements | [#1311](https://github.com/ubugeeei-prod/uf/issues/1311) |
| Number field | Shipped: `NumberField` with locale parsing/formatting, keyboard and button stepping, bounds/step validation and `Field` integration | [#1313](https://github.com/ubugeeei-prod/uf/issues/1313) |
| Table | Shipped for the package's table contract: sort announcements, mixed selection and row counts | Held by the table block in `packages/ui/ui.test.js` |
| Drag/drop and color controls | Shipped: `useDragAndDrop`, collection reorder targets and `ColorPicker` with hex fields, native color input and RGBA channels | [#1310](https://github.com/ubugeeei-prod/uf/issues/1310) |
| Locale and direction infrastructure | Shipped: `I18nProvider`, `useLocale`, `useCollator` and `useFilter`; locale typeahead and mirrored horizontal navigation | [#1312](https://github.com/ubugeeei-prod/uf/issues/1312) |
