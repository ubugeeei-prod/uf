...
The root entry exports namespaces for app code:

```js
import { Dialog, Field, Select } from "@uniflowed/ui";
```

Every primitive also has a subpath with flat names, which is useful for presets,
wrappers and tests that want one dependency edge:

```js
import { DialogRoot, DialogTrigger } from "@uniflowed/ui/dialog";
```

The subpath names are the module names in the table above. `internal/` is not
exported; helpers such as roving focus and prop composition are part of the
primitive contracts, not a public toolkit for building a second library beside
this one.
