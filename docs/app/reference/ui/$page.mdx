...
[`uf ui add`](/reference/cli) writes styled components into a project, each one
built on the parts above and owned by the project from then on;
[Owned or headless](/guide/ui#owned-or-headless) says when to reach for which.
Every component the registry has is below: the command that writes it, its
example running on this page, and its two files, folded under it.

The list is `registry/ui/` itself, read when this site is built from the same
directory `uf` embeds, so a component `uf ui list` offers is a component shown
here. The examples render in the tokens' default theme whatever this site's
theme is, because that is what a project sees before it applies one of its own.

<Registry />

## What this page is

A reference for what `@uniflowed/ui` guarantees, not a gallery. The one look on
it is under [Registry components](#registry-components), since those are files
a project owns rather than parts of the package. The guarantees
