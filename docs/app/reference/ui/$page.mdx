...
A few entries are **blocks** rather than components: a finished piece of a page,
such as `sign-in-form`, made only of components the registry already has.
`uf ui add sign-in-form` writes the block and the components it is made of into
the same directory, and from then on the block is the project's, like any
component. Blocks are Experimental, which means their props may still change
between releases. `uf ui list --json` marks them `"kind": "block"`.
