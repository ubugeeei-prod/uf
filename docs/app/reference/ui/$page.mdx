...
```js
// @flow
// One entry point: a family as a namespace, or a part by its own name.
// `Select.Root` and `SelectRoot` are the same component.
import { Select, SelectRoot } from "@uniflowed/ui";
```
...
`@uniflowed/ui` is the package's one entry point. It exports namespaces for app
code:

```js
import { Dialog, Field, Select } from "@uniflowed/ui";
```

and every part under its own name as well, which suits presets, wrappers and
tests that want one part:

```js
import { DialogRoot, DialogTrigger } from "@uniflowed/ui";
```

There is no subpath to import instead. A bundler keeps the modules in the table
