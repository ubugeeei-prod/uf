...
are skipped. Arrow keys, Home/End, collator-backed typeahead, Shift ranges,
Space/Enter and Ctrl/Cmd+A operate on enabled rows. Trees additionally accept
`expandedKeys`, `defaultExpandedKeys` and `onExpandedChange`; horizontal arrows
expand/collapse and follow the text direction. Tag groups use `onRemove(key)`
for their remove buttons and Delete/Backspace commands.
