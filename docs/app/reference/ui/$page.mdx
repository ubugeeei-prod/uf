...
Sections for `Dialog` and `Tabs` are still to be written, and `Combobox` has
only its groups and the palette they make possible; until they are, each
module's header is the documentation, and it is written to be read.
