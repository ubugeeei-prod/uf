...
it. The list is `registry/ui/` itself, read when this site is built from the
...
| Contract | Held by |
| --- | --- |
| Composition types reject the wrong child before a browser exists | `tests/type-tests/composition.js`, run from `packages/ui/ui.test.js` |
...
| A caller's `className`, `id`, `data-*`, `aria-*`, handlers and refs arrive without replacing the component's own behaviour | the prop-composition block in `packages/ui/ui.test.js` |
| `render` exists only where it is implemented, and every exported part is classified | the render-hatch table in `packages/ui/ui.test.js` |
...
The `packages/ui` tests cover the shipped behaviours, but component-wide
...
The exact source of truth is the audited table in `packages/ui/ui.test.js`; it
...
are stated as tests in `packages/ui/ui.test.js`, where every assertion is
