...
| Package | What it is |
| --- | --- |
| `@uniflowed/fetch` | Typed HTTP — [Data in the browser](/guide/data) |
| `@uniflowed/query` | Caching, revalidation, request de-duplication — [Data in the browser](/guide/data) |
...
| `@uniflowed/form` | Forms: uncontrolled inputs, typed values, narrow subscriptions — [the guide](/guide/form) |
| `@uniflowed/immer` | Immutable updates written as mutations of a draft: `produce`, `produceWithPatches` and `applyPatches`, with results frozen unless `setAutoFreeze(false)` |
| `@uniflowed/cell` | Fine-grained reactivity: `state` and `derived` — [the layer below](/guide/state#the-layer-below) |
| `@uniflowed/validator` | Schemas that parse rather than assert — [Validating input](/guide/validation) |
...
| `@uniflowed/react-testing` | React Testing Library's surface, over a happy-dom window it installs on the first render |
| `@uniflowed/react-native-testing` | React Native test-tree queries. Rendering still waits for the native host config |
