...
Thirteen modules. Each is a separate subpath, so importing `@uniflowed/std/hex`
...
functions raise `NativeRuntimeRequiredError`, and it is not what these thirteen are.
...
`csv`, `binary`, `path` and `glob` are the first next-tranche pieces. What remains,
