...
Twenty modules. Each is a separate subpath, so importing `@uniflowed/std/hex`
...
| `@uniflowed/std/tar` | `archive/tar` | ustar archive reading and writing for regular files and directories |

The root `@uniflowed/std` is unchanged: it is a declaration surface whose
functions raise `NativeRuntimeRequiredError`, and it is not what these twenty are.
...
| --- | --- | --- |
| Node.js | **runs** | `npm/std/std.test.js`, over every shipped module |
...
| `database/sql` | Application-owned driver; queries generated by [sqlc's Flow target](/guide/sqlc) (Experimental) |
...
`csv`, `binary`, `path`, `glob`, `hash`, `time`, `io`, `bufio`, `textproto`, `zip` and `tar` are the first next-tranche
pieces. What remains is still useful, but no next-tranche module is left waiting on this page.
...
its export list. Four tests now hold the statuses to `npm/std`: the
