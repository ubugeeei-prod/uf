...
Put translated pages beneath `app/[locale]/`. One shared module supplies the
supported locale union to middleware, the prerender and components:
...
The middleware redirects GET and HEAD requests for `/`, preferring a supported
`uf.locale` cookie and then `Accept-Language`. It preserves the query, varies on
both headers and uses `private, no-store`; unsupported cookie values cannot
become redirect destinations. Requests already beneath a locale pass through.
...
export function loader({ params }) {
  if (!locales.locales.includes(params.locale)) notFound();
  return null;
}

export default component Page(params: { locale: string }) {
  const locale = locales.locale(params);
  return <main>{locale === "ja" ? "こんにちは" : "Hello"}</main>;
}
```

The same typed accessor works in server and client components. For a nested
page, `locales.metadata("/guide")` emits reciprocal `hreflang` URLs, including
`x-default`. Set `site.url` in `uf.config.js`; the regular prerender writes both
locale documents and includes each in `sitemap.xml`. Enumerate the locales for
each dynamic page through `generateStaticParams`. The loader rejects unsupported
locale paths with a 404 before the typed accessor is called. This also lets the
build render the root not-found page when a dynamic locale segment matches its
probe URL. Client components can pass `useRoute().params` to the same accessor.
...
extracted when you ask. [`uf i18n extract`](/reference/cli#uf-i18n-extract)
...
accepts; [`uf i18n merge`](/reference/cli#uf-i18n-merge-file) reads the
