...
export default component Page() {
  const locale = locales.locale(useRoute().params);
...
each dynamic page through `generateStaticParams`. An unsupported locale is an
error; call the router's `notFound()` if it should render a 404.
