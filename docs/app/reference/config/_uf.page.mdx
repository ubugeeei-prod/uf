...
`rules` names the rules you want to change and nothing else. Naming one used to
*replace* the table, so a project that lowered a single rule to a warning
switched the other fifty-three off — including `flow/syntax`, so it also stopped
being told its sources do not parse
([#475](https://github.com/ubugeeei-prod/uf/issues/475)). Switching a rule off
is what `"off"` is for.

A name that is not a rule is refused, with the closest one uf does have:

```
error: `flow/unclear-types` is not a lint rule; did you mean `flow/unclear-type`?
```

because a misspelled rule that quietly does nothing is the same failure one
level down.
