...
| `runner` | `"uf"` | The runner. `"uf"` is the one built into uf. `"bun@1.4"` is read, implies Bun 1.4 as the test runtime, and is refused by `uf test` until [#942](https://github.com/ubugeeei-prod/uf/issues/942); beside a `runtime` naming anything else it is an error naming both keys |
