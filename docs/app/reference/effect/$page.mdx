---
title: "Effect API · uf"
description: "Typed failures, fibers that own what they start, and what Flow cannot say."
---
...
This page is the contract, export by export. [Effects](/guide/effect), in the
guide, is the walk-through — composing, recovering, `race` and `all`, and how
it maps from Effect-TS — and is not repeated here.

```js
// @flow
import { effect, fail, promise, retry, runPromise, tryPromise } from "@uniflowed/effect";
...
is `setTimeout`; tracing, spans, metrics and the logging layer; a typed defect
channel; a `Context` module, since `tag`, `provideService` and `provide` are the
whole surface and the context they thread is internal; and a heterogeneous
`all`, which in Effect-TS keeps a tuple's element types and here takes and
returns one array type.

Nor is there a `pipe` or typeclass dispatch: Effect-TS is built on a
higher-kinded encoding, and Flow has no higher-kinded types — a type parameter
cannot itself take type arguments. Composition is data-first functions over
`Effect`, a smaller surface with nothing to resolve at run time;
[Coming from Effect-TS](/guide/effect#coming-from-effect-ts) maps each
Effect-TS idiom onto what is here.
