...
choice**: uf reads `app.runtime.capabilityJsHost`, the same setting that
decides which host `uf dev` and `uf test` run on, and uses the first of the
accepted hosts that can build a binary. Running one still needs nothing.
