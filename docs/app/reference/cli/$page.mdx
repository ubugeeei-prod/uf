...
still in your checkout — no `node_modules`, no source, no `uf`. Six targets
are implemented. Five of them run an application and share every line of it;
the sixth runs none:
...
`handler.js` is the seam and is identical work in the five that have one: it is
...
five server targets and the project does not have to change to use one. One
...
`deno` is a name in `DeployAdapter` and nothing else. Asking for it is an error
that says why, and names
[#391](https://github.com/ubugeeei-prod/uf/issues/391).

| Target | Why not |
| --- | --- |
| `deno` | The `node` output already runs unchanged on Deno. An adapter here is `Deno.serve` in place of `node:http`, and it is worth writing only once a benchmark shows the native server beating `node:http` *under the same handler* — otherwise it is a directory with a different name on it. That benchmark has been run for Bun and not for Deno, on a machine that has no Deno. |
