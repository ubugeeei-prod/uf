...
| --- | --- | --- | --- | --- | --- |
| `uf link` | `npm link` | `pnpm link` | `yarn link` | refused | `bun link` |
| `uf link NAME` | `npm link NAME` | `pnpm link NAME` | `yarn link NAME` | refused | `bun link NAME` |
| `uf link DIR` | `npm link DIR` | `pnpm link DIR` | refused | `yarn link DIR` | refused |

Yarn 1 and bun link by name only and Yarn 2+ by path only, and each refusal names
the form that manager has. Linking installs, so dependency scripts stay refused:
`--ignore-scripts`, or `--config.ignore-scripts=true` for `pnpm link`, which
rejects the flag. Linking a directory or a name is reported like `uf add`, with
the manifest and the tree; registering changes nothing in the project, so it
prints the three rows and the manager's own answer.
