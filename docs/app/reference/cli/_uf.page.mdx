...
`--size-report` prints the cost of each chunk. Two flags write something you
can take away with you: `--adapter TARGET` writes a directory that runs on a
host with a JavaScript runtime and nothing else, and `--compile` writes a
single executable file that needs not even that. Both are described below,
beside the two ways to serve a build in place.
