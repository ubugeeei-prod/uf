...
redirection, `$VAR`, a backtick, an unquoted `*`, a leading `~`, a newline, or
a first word that is a shell keyword or a built-in with no program behind it
(`cd`, `exit`, `export`) — is handed to `sh -c`, exactly as before. On a
