...
0` → `export const count = 0` (`flow/non-const-var-export`) is the one uf has:
the file still parses and still formats, and it throws at run time if anything
reassigns the binding. It is not a guess at intent — a fix uf cannot spell at
all is in neither tier — but it can change what the program does, so it waits
to be asked for by name. Read the diff before committing it.
